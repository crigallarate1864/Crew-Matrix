import {api} from './api.js?v=1.0.0-HOTFIX-0012';
import {
  readSession,
  storeSession,
  routeForProfile,
...
function ensureSendOverlay(){
  let overlay=$('#volunteerSendOverlay');if(overlay)return overlay;
  overlay=document.createElement('div');overlay.id='volunteerSendOverlay';overlay.className='volunteer-send-overlay';
  overlay.innerHTML=`<section class="volunteer-send-card" role="status" aria-live="polite" aria-busy="true"><span>ATLAS 118 · Referente Volontari</span><strong>Invio richieste al RO…</strong><p>Salvo le richieste e attendo la conferma del server. La bozza viene cancellata soltanto a conferma ricevuta.</p><div class="volunteer-send-progress"><i></i></div><div class="volunteer-send-actions"><button class="portal-btn" id="volunteerSendCancelBtn" type="button">Annulla</button></div></section>`;
  document.body.appendChild(overlay);
  overlay.querySelector('#volunteerSendCancelBtn')?.addEventListener('click',()=>{sendCancelled=true;sendController?.abort();overlay.classList.remove('open');});
  return overlay;
}
function showSendOverlay(){ensureSendOverlay().classList.add('open');}
function hideSendOverlay(){$('#volunteerSendOverlay')?.classList.remove('open');}
...
async function sendAll(){
  ...
  sendCancelled=false;sendController=new AbortController();
  button.disabled=true;button.textContent='Salvataggio e invio…';showSendOverlay();
  ...
  try{
    const result=await api({
      ...
      sendEmail:true
    },{timeout:60000,signal:sendController.signal});
    ...
    const cancelled=error?.name==='AbortError'||sendCancelled;
    $('#reviewResult').innerHTML=cancelled?`<div class="notice info"><strong>Invio annullato.</strong><br>La bozza è rimasta disponibile e puoi riprovare.</div>`:`<div class="notice danger"><strong>Invio non riuscito.</strong><br>${esc(error.message)}</div>`;
    button.disabled=false;
  }finally{
    sendController=null;hideSendOverlay();button.textContent='Invia definitivamente al RO';
  }
}
