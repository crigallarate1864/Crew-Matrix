      ...
      <div class="proposal-body">
        ...
        ${canReview&&!ready?`
          <div class="notice warning">
            ...
            Genera e salva prima il calendario dipendenti di ${esc(month)}.
          </div>`:''}
      ...
      ${canReview?`
        <div class="proposal-actions">
          <button class="btn small primary"
                  ${ready?`data-review="${esc(item.id)}" data-decision="APPROVE"`:'disabled'}>
            Approva
          </button>
          <button class="btn small danger"
                  ${ready?`data-review="${esc(item.id)}" data-decision="REJECT"`:'disabled'}>
            Rigetta
          </button>
        </div>`:''}
    </article>
...
export async function refreshWorkspace(){
  ...
  const host=$('#volunteerProposalList');
  if(host)host.innerHTML='<div class="volunteer-loading">Caricamento proposte…</div>';
