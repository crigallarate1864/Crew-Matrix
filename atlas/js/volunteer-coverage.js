...
function proposalCard(p){
  const h=proposalHole(p),
    sol=proposalSolution(p),
    pendingReview=
      has('volunteerProposalReview')&&
      p.status==='INVIATA',
    calendarReady=p.calendarReady===true,
    canReview=pendingReview&&calendarReady,
    calendarMonth=
      p.calendarMonth||
      String(h.day||'').slice(0,7);

  const crew=(sol.crew||[])
    .map(x=>
      `${esc(x.role)}: <strong>${esc(x.employeeName)}</strong>`
    )
    .join(' · ');

  const changes=(sol.changes||[])
    .map(x=>
      `<div class="solution-change">↔ ${esc(x.fromEmployeeName)} → ${esc(x.toEmployeeName)} · ${esc(x.code)} ${esc(x.day)} ${esc(x.start)}–${esc(x.end)}</div>`
    )
    .join('');

  const calendarGate=pendingReview&&!calendarReady
    ?`<div class="notice warning">
        <strong>Valutazione bloccata.</strong>
        Prima di approvare o rifiutare questa proposta, il Responsabile Operativo
        deve generare e salvare il calendario dipendenti di
        <strong>${esc(calendarMonth)}</strong>.
      </div>`
    :pendingReview
      ?`<div class="notice success">
          <strong>Calendario mensile verificato.</strong>
          Generato e salvato${p.calendarSavedAt?` il ${esc(p.calendarSavedAt)}`:''}.
        </div>`
      :'';
  ...
  return `<article class="proposal-card proposal-immutable">
    <div class="proposal-head">
      <div>
        <div class="proposal-title">${esc(h.day)} · ${esc(h.shift)} · ${esc(h.site)}${h.machine?` · mezzo ${esc(h.machine)}`:''}</div>
        <div class="proposal-meta">Proposta ${esc(p.id)} · ${esc(p.createdByDisplay||p.createdBy)} · ${esc(p.createdAt)}${p.emailSentAt?` · email RO inviata ${esc(p.emailSentAt)}`:''}</div>
      </div>
      <span class="proposal-status ${statusClass(p.status)}">${esc(p.status)}</span>
    </div>
    <div class="proposal-body">
      <div class="proposal-line">Ruoli richiesti: <strong>${esc((h.roles||[]).join(', '))}</strong></div>
      <div class="proposal-line">Copertura proposta: ${crew||'—'}</div>
      ${h.note?`<div class="proposal-line">Nota referente: <strong>${esc(h.note)}</strong></div>`:''}
      ${changes?`<div class="solution-changes"><strong>Cambi turno proposti</strong>${changes}</div>`:''}
      ${p.reviewReason?`<div class="proposal-line">Esito RO: <strong>${esc(p.reviewReason)}</strong></div>`:''}
      ${calendarGate}
    </div>
    ${actions}
  </article>`;
