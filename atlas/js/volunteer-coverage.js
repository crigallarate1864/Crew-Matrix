// ATLAS 118 · Buchi volontari SMART REPORT 2242
  ...
  const changes=operations.map(operation=>{
    ...
    if(operation.mode==='direct'){
      return{
        type:'DIRECT',
        role:String(operation.role||''),
        text:`${operation.coverName||'Risorsa'} copre direttamente ${roleLabel(operation.role)}`
      };
    }
    return{
      type:'COVERED',
...
function ensureCompatibilityToolbar(){
  ...
  toolbar.innerHTML=`
    ...
    <div class="accepted-toolbar">
      ...
      <button class="btn small accepted-report-btn"
              id="acceptedReportBtn"
              type="button">
        Report modifiche
      </button>
