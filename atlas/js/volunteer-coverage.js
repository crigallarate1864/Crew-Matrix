...
function renderHoursScore(solution){
  ...
  if(!rows.length){
    return`
      <aside class="hours-score-panel clean">
        <div class="hours-score-title">
          Impatto ore
        </div>

        <div class="hours-score-empty">
          Nessuna variazione del monte ore.
        </div>
      </aside>
    `;
  }
  ...
  return`
    <aside class="hours-score-panel clean">
      <div class="hours-score-head">
        ...
        <span class="hours-total-badge hours-balanced">
          dopo il cambio
        </span>
      </div>

      <div class="hours-score-explain">
        ATLAS mostra direttamente quante ore avrà ogni persona
          ...
          return`
            <div class="hours-score-row">
              ...
              <div class="hours-score-values">
                <span class="hours-delta ${hourBalanceClass(row)}">
                  ${esc(row.label)}
                </span>

                <small>
                  ${formatHours(row.plannedBefore)}
                  → <strong>${formatHours(row.projected)} h</strong>
                  · target ${formatHours(row.target)} h
                </small>
              </div>
            </div>
          `;
