...
function solutionHoursOptionLabel(solution){
  const coverers=(solution.operations||[])
    .filter(operation=>
      operation.coverHours&&
      operation.mode!=='covered'
    )
    .map(operation=>
      `${operation.coverName} ${operation.coverHours.label}`
    );

  return coverers.length
    ?` · ore: ${coverers.join(' / ')}`
    :'';
}
...
function renderHoursScore(solution){
  ...
  (solution.operations||[])
    .forEach(operation=>{
      if(
        ...
        rows.push({
          key:
            `cover-${operation.role}-${operation.coverEmployeeId}`,
          name:operation.coverName,
          context:
            `${roleLabel(operation.role)} · copre il buco`,
          snapshot:operation.coverHours,
          primary:true
        });
      }
      ...
      if(
        ...
        operation.replacementHours
      ){
        rows.push({
          key:
            `replace-${operation.role}-${operation.replacementEmployeeId}`,
          name:operation.replacementName,
          context:
            `Sostituisce sul turno ${operation.sourceCode}`,
          snapshot:operation.replacementHours,
          primary:false
        });
      }
    });
  ...
  if(!rows.length){
    return`
      <aside class="hours-score-panel">
        <div class="hours-score-title">
          Monte ore risorse
        </div>

        <div class="hours-score-empty">
          Nessuna nuova risorsa da confrontare.
        </div>
      </aside>
    `;
  }
  ...
  const missing=
    Number(
      solution.coverageMissingHours||
      0
    );

  return`
    <aside class="hours-score-panel">
      <div class="hours-score-head">
        <div>
          <div class="hours-score-kicker">
        ...
        <span class="hours-total-badge ${
          ...
          ${
            missing>0
              ?`${formatHours(missing)} h mancanti`
              :'Target raggiunto'
          }
        </span>
      </div>

      <div class="hours-score-explain">
        Più il valore è negativo, più ore restano da programmare
        a quella persona nel mese aperto.
      </div>

      <div class="hours-score-list">
        ${rows.map(row=>`
          <div class="hours-score-row ${
            row.primary
              ?'primary-resource'
            ...
            <div class="hours-score-person">
              ...
              <span>${esc(row.context)}</span>
            </div>

            <div class="hours-score-values">
              <span class="hours-delta ${hourBalanceClass(row.snapshot)}">
                ${esc(row.snapshot.label)}
              </span>

              <small>
                ${formatHours(row.snapshot.planned)}
                / ${formatHours(row.snapshot.target)} h
              </small>
            </div>
          </div>
        `).join('')}
      ...
      <div class="hours-score-legend">
        <span>
          <i class="score-dot missing"></i>
          ore da recuperare
        </span>
        <span>
          <i class="score-dot balanced"></i>
          in target
        </span>
        <span>
          <i class="score-dot excess"></i>
          ore in eccesso
        </span>
      </div>
    </aside>
      ...
      <div class="solution-decision-grid">
        <div class="solution-choice-column">
            ...
            ${solutions
              .map((solution,index)=>`
                <option value="${esc(solution.signature)}"
                  ...
                  ${index+1}. ${esc(solution.label)}${esc(solutionHoursOptionLabel(solution))}
                </option>
              `)
              .join('')
            }
          ...
          <div class="solution-help">
            ATLAS ordina prima per compatibilità e minore invasività.
            Il monte ore a destra serve come supporto alla scelta del RO.
          </div>
        </div>
