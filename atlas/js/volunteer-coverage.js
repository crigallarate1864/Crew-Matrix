...
function ensureCompatibilityStyles(){
  ...
  style.textContent=`
    .volunteer-analysis-overlay{
      ...
      place-items:center;
      padding:24px;
      background:rgba(3,12,18,.78);
      backdrop-filter:blur(12px) saturate(120%);
      -webkit-backdrop-filter:blur(12px) saturate(120%)
    }

    .volunteer-analysis-overlay.hidden{
      display:none!important
    }

    .volunteer-analysis-card{
      width:min(470px,calc(100vw - 36px));
      padding:24px 25px 22px;
      border:1px solid rgba(103,200,230,.20);
      border-radius:20px;
      background:linear-gradient(160deg,#0b2432,#071821 72%);
      box-shadow:0 32px 90px rgba(0,0,0,.55),inset 0 1px 0 rgba(255,255,255,.04)
    }

    .volunteer-analysis-head{
      display:flex;
      align-items:center;
      gap:15px
    }

    .volunteer-analysis-spinner{
      position:relative;
      width:48px;
      height:48px;
      flex:0 0 auto;
      border:3px solid rgba(255,255,255,.10);
      border-top-color:#39c7ea;
      border-radius:50%;
      animation:atlasVolunteerSpin .85s linear infinite
    }

    .volunteer-analysis-spinner::after{
      content:'';
      position:absolute;
      inset:8px;
      border:2px solid rgba(255,255,255,.08);
      border-bottom-color:#e3263e;
      border-radius:50%;
      animation:atlasVolunteerSpin 1.2s linear infinite reverse
    }

    @keyframes atlasVolunteerSpin{
      to{transform:rotate(360deg)}
    }

    .volunteer-analysis-kicker{
      color:#6ea9bc;
      font-size:10px;
      font-weight:900;
      letter-spacing:.12em;
    ...
    .volunteer-analysis-progress > i{
      display:block;
      width:8%;
      height:100%;
      border-radius:inherit;
      background:linear-gradient(90deg,#39c7ea,#8bdcf0);
      transition:width .18s ease
    }

    .volunteer-analysis-foot{
      display:flex;
    ...
    body.volunteer-analysis-locked{
...
function ensureVolunteerAnalysisOverlay(){
  ...
  overlay.className='volunteer-analysis-overlay hidden';
  overlay.innerHTML=`
    <div class="volunteer-analysis-card" role="status" aria-live="polite" aria-busy="true">
      <div class="volunteer-analysis-head">
        <div class="volunteer-analysis-spinner" aria-hidden="true"></div>
        <div>
          <div class="volunteer-analysis-kicker">ATLAS 118 · Pianificatore</div>
          <div class="volunteer-analysis-title" id="volunteerAnalysisTitle">Carico Buchi volontari</div>
          <div class="volunteer-analysis-text" id="volunteerAnalysisText">Recupero le richieste e preparo l'analisi del calendario.</div>
        </div>
      </div>
      <div class="volunteer-analysis-progress"><i id="volunteerAnalysisBar"></i></div>
      <div class="volunteer-analysis-foot">
        <span id="volunteerAnalysisStage">Preparazione…</span>
        <span id="volunteerAnalysisCounter"></span>
      </div>
    </div>
  `;
...
function showVolunteerAnalysisOverlay({
  ...
  text='Recupero le richieste e preparo l’analisi del calendario.',
  stage='Preparazione…',
  current=0,
  total=0
}={}){
  ...
  document.body.classList.add('volunteer-analysis-locked');
  updateVolunteerAnalysisOverlay({title,text,stage,current,total});
}

function updateVolunteerAnalysisOverlay({
  title,
  text,
  stage,
  current,
  ...
  const overlay=ensureVolunteerAnalysisOverlay();
  const titleEl=overlay.querySelector('#volunteerAnalysisTitle');
  const textEl=overlay.querySelector('#volunteerAnalysisText');
  const stageEl=overlay.querySelector('#volunteerAnalysisStage');
  const counterEl=overlay.querySelector('#volunteerAnalysisCounter');
  const bar=overlay.querySelector('#volunteerAnalysisBar');

  if(title!==undefined&&titleEl)titleEl.textContent=title;
  if(text!==undefined&&textEl)textEl.textContent=text;
  if(stage!==undefined&&stageEl)stageEl.textContent=stage;

  const safeTotal=Math.max(0,Number(total||0));
  const safeCurrent=Math.max(0,Math.min(safeTotal,Number(current||0)));
  const percent=safeTotal?Math.max(8,Math.round((safeCurrent/safeTotal)*100)):12;

  if(bar)bar.style.width=`${percent}%`;
  if(counterEl){
    counterEl.textContent=safeTotal?`${safeCurrent}/${safeTotal}`:'';
  }
}

function hideVolunteerAnalysisOverlay(){
  const overlay=$('#volunteerAnalysisOverlay');
  overlay?.classList.add('hidden');
  document.body.classList.remove('volunteer-analysis-locked');
}
...
function optimizeGlobalProposalPlan(pending){
  // FAST GLOBAL PLAN — greedy "most constrained first".
  // Evita completamente il backtracking esponenziale sul thread UI.
  const eligible=(pending||[])
    ...
    .filter(entry=>
      ['DIRECT','CHANGES'].includes(entry.result?.status)&&
      (entry.result?.solutions||[]).length
    )
    .map(entry=>({
      ...entry,
      solutions:(entry.result.solutions||[])
        .slice(0,5)
        .map(solution=>({
          solution,
          footprint:proposalSolutionFootprint(entry.item,solution),
          quality:
    ...
    .sort((a,b)=>
      ...
      a.item.id.localeCompare(b.item.id,'it')
    );

  const used=new Set();
  const selectedMap=new Map();

  // Primo passaggio: assegna la soluzione migliore alle richieste più vincolate.
  for(const entry of eligible){
    const candidate=entry.solutions.find(option=>
      ![...option.footprint].some(key=>used.has(key))
    );
    if(!candidate)continue;
    selectedMap.set(entry.item.id,candidate.solution.signature);
    candidate.footprint.forEach(key=>used.add(key));
  }

  // Secondo passaggio leggero: prova a recuperare richieste rimaste fuori
  // usando una soluzione alternativa non in conflitto.
  for(const entry of eligible){
    if(selectedMap.has(entry.item.id))continue;
    const candidate=entry.solutions.find(option=>
      ![...option.footprint].some(key=>used.has(key))
    );
    if(!candidate)continue;
  ...
  eligible.forEach(entry=>{
    const signature=selectedMap.get(entry.item.id)||'';
    entry.result.globalRecommendedSignature=signature;
    entry.result.globalPlanSelected=Boolean(signature);
    entry.result.globalConflict=!signature;
    if(signature)selectedSolutions.set(entry.item.id,signature);
    else selectedSolutions.delete(entry.item.id);
  });

  const impossible=(pending||[]).filter(item=>
    compatibility.get(item.id)?.status==='INCOMPATIBLE'
  ).length;
  const review=(pending||[]).filter(item=>
    compatibility.get(item.id)?.status==='REVIEW'
  ).length;

  globalPlanStats={
    total:(pending||[]).length,
    covered:selectedMap.size,
    impossible,
    review,
    conflict:Math.max(0,eligible.length-selectedMap.size)
  };
}
...
async function idleYield(){
  // Cede SEMPRE il thread al browser per aggiornare overlay/progresso.
...
async function runCompatibilityAnalysis({
  ...
  const analyze=analyzer()?.analyzeProposal;
  ...
  if(blocking){
    showVolunteerAnalysisOverlay({
      title:'Analizzo Buchi volontari',
      text:'Controllo l’intero calendario e cerco le coperture più logiche.',
      stage:'Analisi compatibilità',
      current:0,
      total:pending.length
    });
  }
  ...
  analysisBusy=true;
  const analysisDeadline=Date.now()+6500;

  try{
    if(
      !force&&
      pending.length&&
      pending.every(item=>compatibility.has(item.id))
    ){
      if(blocking){
        updateVolunteerAnalysisOverlay({
          title:'Ottimizzo il piano',
          text:'Le singole richieste sono già analizzate. Verifico la combinazione globale migliore.',
          stage:'Ottimizzazione complessiva',
          current:pending.length,
          total:pending.length
        });
      }

      optimizeGlobalProposalPlan(pending);
      render();
      return;
    }
    ...
    for(const item of pending){
      ...
      if(Date.now()>analysisDeadline){
        // Nessun freeze: ciò che non è stato analizzato entro il budget
        // resta esplicitamente DA VERIFICARE, mai IMPOSSIBILE.
        for(const remaining of pending.slice(processed)){
          if(!compatibility.has(remaining.id)||compatibility.get(remaining.id)?.status==='ANALYZING'){
            compatibility.set(remaining.id,{
              status:'REVIEW',
              label:'Da verificare',
              tone:'warning',
              summary:'Analisi automatica interrotta dal limite di sicurezza per mantenere ATLAS reattivo.',
              roles:[],changes:[],solutions:[],
              blockers:['Richiesta non dichiarata impossibile: è necessaria una verifica manuale o un nuovo tentativo.'],
      ...
      try{
        // Lascia al browser un frame per aggiornare il popup/progresso prima
        // della ricerca CPU-bound della singola proposta.
        await idleYield();
        compatibility.set(item.id,analyze(item));
      }catch(error){
        compatibility.set(item.id,{
          status:'REVIEW',
          label:'Da verificare',
          tone:'warning',
          summary:'ATLAS non è riuscito a completare il controllo automatico. La richiesta non viene classificata come impossibile.',
          roles:[],
          changes:[],
          solutions:[],
      ...
      if(blocking){
        updateVolunteerAnalysisOverlay({
          title:'Analizzo Buchi volontari',
          text:'Valuto disponibilità, riposi, qualifiche, ore e possibili catene di cambio.',
          stage:'Analisi compatibilità',
          current:processed,
          total:pending.length
        });
      ...
      if(processed%3===0||processed===pending.length){
        render();
      }
      await idleYield();
    }
    ...
    if(blocking){
      updateVolunteerAnalysisOverlay({
        title:'Ottimizzo il piano',
        text:'Confronto le soluzioni per coprire il maggior numero possibile di richieste con il minor numero di modifiche.',
        stage:'Piano complessivo',
        current:pending.length,
        total:pending.length
      });
      await idleYield();
...
export async function refreshWorkspace({
  ...
  if(blocking){
    showVolunteerAnalysisOverlay({
      title:'Carico Buchi volontari',
      text:'Recupero le proposte ricevute dal Responsabile Volontari.',
  ...
  if(host){
    host.innerHTML=`
      <div class="volunteer-loading">
        ...
        <span>Leggo le richieste volontari e preparo il calendario per l’analisi.</span>
      </div>
    `;
  }
    ...
    try{
      ...
      if(blocking){
        updateVolunteerAnalysisOverlay({
          ...
          text:'Ora verifico tutte le richieste sul calendario completo.',
          stage:'Avvio analisi',
          current:0,
          total:(workspace.proposals||[]).filter(item=>item.status==='INVIATA').length
        });
      }

      render();
      await runCompatibilityAnalysis({force,blocking});
      return workspace;
    }finally{
