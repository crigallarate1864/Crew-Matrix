...
(() => {
  ...
  function rankCandidates(day,item,{preferred=null,allowRo=false,pool=null,preferMale=false,preserveMale=false,requireMale=false}={}){
    ...
    list=list.filter(e=>checkCandidate(e,day,item,{allowRo}).errors.length===0);
    if(!allowRo)list=list.filter(e=>e.turno!=='RO');
    return list.map(e=>({e,score:candidateScore(e,day,item,preferred,{preferMale,preserveMale})})).sort((a,b)=>a.score-b.score||employeeName(a.e).localeCompare(employeeName(b.e)));
  }
  function chooseCandidate(day,item,options={}){return rankCandidates(day,item,options)[0]?.e||null;}
  ...
  function finishAutomaticGeneration(summary,rfsPlacement=null){
    ...
    toast(
      ...
      `${summary.added} assegnazioni proposte · ${summary.missing} ruoli scoperti · ${summary.cross} cross A/B segnalati${summary.emergencyCoverage?` · ${summary.emergencyCoverage} coperture emergenziali RO/RS`:''} · ${summary.weeklyBalance?.moved||0} spostamenti per limite settimanale · ${summary.genderSwaps} riequilibri equipaggio${summary.weeklyBalance?.unresolved?` · ${summary.weeklyBalance.unresolved} settimane sovraccariche non risolte`:''}${restText}${rfsText}.`,
  ...
  function coverageCandidatePool({emergency=false}={}){
    if(emergency)return state.employees.filter(e=>e.turno!=='Amministrazione');
    return state.employees.filter(e=>['A','B','Libera'].includes(e.turno));
  }
  function slotCandidateRanking(day,shift,slot,targetGroup,{emergency=false}={}){
    ...
    return rankCandidates(day,item,{preferred:targetGroup,allowRo:emergency,pool:coverageCandidatePool({emergency}),preferMale:!hasMale,preserveMale:hasMale&&otherCrewNeedsMale,requireMale}).map(x=>({...x,item}));
  }
  function placeCoverageTrial(employee,day,item){
    const crossGroup=['A','B'].includes(employee.turno)&&employee.turno!==item.plannedGroup;
    const assignment=autoItem({...item,crossGroup,crossGroupFrom:crossGroup?item.plannedGroup:'',crossGroupTo:crossGroup?employee.turno:'',crossReason:crossGroup?`Copertura completa: gruppo ${employee.turno} utilizzato sul turno ${item.plannedGroup}.`:'',note:crossGroup?`Cross A/B automatico per evitare una scopertura: gruppo ${employee.turno} utilizzato sul turno ${item.plannedGroup}.`:''});
    const key=assignmentKey(employee.id,day);
    state.assignments[key]=[...(state.assignments[key]||[]),assignment];
    refreshAutoCache();
    return{employee,day,assignment,key,crossGroup};
  }
  function removeCoverageTrial(trial){
    const keep=(state.assignments[trial.key]||[]).filter(a=>a.id!==trial.assignment.id);
    if(keep.length)state.assignments[trial.key]=keep;else delete state.assignments[trial.key];
    refreshAutoCache();
  }
  function solveRequiredShift(day,shift,targetGroup,{emergency=false,maxNodes=50000}={}){
    ...
    function search(slots,trials){
      ...
      const ranked=slots.map(slot=>({slot,candidates:slotCandidateRanking(day,shift,slot,targetGroup,{emergency})})).sort((a,b)=>a.candidates.length-b.candidates.length||(['A','C','S'].indexOf(a.slot.role)-['A','C','S'].indexOf(b.slot.role)));
      const current=ranked[0];
      if(!current||!current.candidates.length)return null;
      const next=slots.filter(s=>s!==current.slot);
      for(const candidate of current.candidates){
        const trial=placeCoverageTrial(candidate.e,day,candidate.item);
    ...
    return{success:!!trials,trials:trials||[],nodes,emergency};
  }
  function finalizeCoverageTrials(trials){
    ...
    trials.forEach(trial=>{
      ...
      if(['RO','RS'].includes(trial.employee.turno)){
        emergency++;
        trial.assignment.emergencyCoverage=true;
        trial.assignment.note=[trial.assignment.note,`Copertura emergenziale con ${trial.employee.turno}`].filter(Boolean).join(' · ');
      }
      added++;
    });
    refreshAutoCache();
  ...
  function scheduleShiftCompletely(day,shift,targetGroup){
    let solved=solveRequiredShift(day,shift,targetGroup,{emergency:false});
    if(!solved.success)solved=solveRequiredShift(day,shift,targetGroup,{emergency:true});
    if(!solved.success){
  ...
  async function schedule118({allowRo=false,onProgress=null}={}){
    ...
    for(let i=0;i<dates.length;i++){
      ...
      if(i%2===1||i===dates.length-1){if(onProgress)onProgress(i+1,dates.length);await yieldUi();}
    }
