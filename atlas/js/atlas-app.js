...
(() => {
      ...
      .map(([sourceDay,sourceRow])=>{
        ...
        if(!recovery){
          recovery=recoveries.find(row=>{
            const identity=row.a.id||`${row.day}|${row.a.code}`;
            return !usedRecoveryIds.has(identity)&&!row.a.linkedEventDay&&row.day>sourceDay;
          });
        }

        if(recovery){
          usedRecoveryIds.add(recovery.a.id||`${recovery.day}|${recovery.a.code}`);
        }
        ...
        return{
          ...
          due:addDaysKey(sourceDay,Number(state.settings.holidayRecoveryDays||30)),
          recovery,
          used:!!recovery
        };
      });
  ...
  function rfsCounter(employeeId,year){
    ...
    return{earned,used,remaining:Math.max(0,earned-used),ledger};
  }
  function pendingRfsEntitlements(){
    const year=Number(state.month.slice(0,4));
    const monthEnd=monthDateKeysFor(state.month).at(-1);
    const years=state.month.endsWith('-01')?[year-1,year]:[year];
    const list=[];
      ...
      .forEach(employee=>{
        years.forEach(value=>{
          rfsEntitlementLedger(employee.id,value)
            .filter(item=>!item.used&&item.sourceDay<=monthEnd)
            .forEach(item=>list.push(item));
        });
      });

    const unique=new Map();
    list.forEach(item=>unique.set(`${item.employeeId}|${item.sourceDay}`,item));
  ...
  function validateAll(){
    ...
    const currentYear=Number(state.month.slice(0,4)),today=dateKey(new Date());
    state.employees.forEach(e=>{
      rfsEntitlementLedger(e.id,currentYear)
        .filter(item=>!item.used&&item.sourceDay.startsWith(state.month))
        .forEach(item=>{
          out.push(validation(
            item.due<today?'error':'warning',
            'RFS da programmare',
            `${employeeName(e)} ha maturato un RFS lavorando il ${formatDateIt(item.sourceDay)}. Il recupero risulta ancora disponibile${item.due?` con termine indicativo ${formatDateIt(item.due)}`:''}.`,
            e.id,
            item.sourceDay
          ));
        });
  ...
  function rfsCandidateDays(employee,entitlement,targetMonth,store){
    ...
    const minimum=entitlement.sourceDay>=firstDay
      ?addDaysKey(entitlement.sourceDay,1)
      :firstDay;

    return monthDateKeysFor(targetMonth)
      .filter(day=>day>=minimum&&!holidays.has(day))
      .map(day=>{
        ...
        const overdue=day>entitlement.due;
        const weekday=date.getDay()>=1&&date.getDay()<=5;
        const distance=Math.max(0,daysBetween(minimum,day));
        return{
          day,key,items,blocked,
          score:(overdue?200:0)+(weekday?0:40)+distance
        };
      })
      .filter(item=>!item.blocked)
      .sort((a,b)=>a.score-b.score||a.day.localeCompare(b.day));
  }
  function createRfsItem(employee,entitlement,targetMonth){
    ...
    return{
      ...
      note:`RFS maturato per festività lavorata del ${formatDateIt(entitlement.sourceDay)}.`,
      createdAt:now,
      updatedAt:now
    };
  }
  function placeRfsEntitlements(entitlements,targetMonth){
    ...
    let placed=0,unresolved=0,overdue=0;
    const details=[];

    entitlements.forEach(entitlement=>{
      const employee=state.employees.find(e=>e.id===entitlement.employeeId);
      if(!employee){unresolved++;return;}

      const existing=Object.entries(store).some(([key,items])=>{
        const split=key.lastIndexOf('|');
        const employeeId=key.slice(0,split);
        return employeeId===employee.id&&(items||[]).some(item=>
          isRfsRecord(item)&&item.linkedEventDay===entitlement.sourceDay
        );
      });
      if(existing)return;

      const candidate=rfsCandidateDays(employee,entitlement,targetMonth,store)[0];
      if(!candidate){
        unresolved++;
        details.push(`${employeeName(employee)} · nessuna giornata disponibile`);
        return;
      }

      const keep=(candidate.items||[]).filter(item=>
        !(sourceLabel(item)==='AUTO'&&!item.locked)
      );
      store[candidate.key]=[
        ...keep,
        createRfsItem(employee,entitlement,targetMonth)
      ];
      placed++;
      if(candidate.day>entitlement.due)overdue++;
      details.push(`${employeeName(employee)} · ${formatDateIt(candidate.day)}`);
    });
    ...
    state.localDirty=true;
    return{placed,unresolved,overdue,targetMonth,details};
  }
  function rfsModalRows(entitlements){
    ...
    return [...grouped.values()]
      ...
      .map(entry=>{
        ...
        const days=entry.pending.map(item=>formatDateIt(item.sourceDay)).join(', ');
        return`<tr>
  ...
  function openRfsPlacementModal(entitlements,context){
    ...
    $('#rfsPlacementSubtitle').textContent=
      `${entitlements.length} RFS da collocare · ${new Set(entitlements.map(item=>item.employeeId)).size} dipendenti`;
    ...
    $('#rfsCurrentMonthLabel').textContent=monthLabel(state.month);
    $('#rfsNextMonthLabel').textContent=monthLabel(nextMonthValue(state.month));
  ...
  function finishAutomaticGeneration(summary,rfsPlacement=null){
    ...
    const rfsText=rfsPlacement
      ?` · ${rfsPlacement.placed} RFS collocati${rfsPlacement.unresolved?` · ${rfsPlacement.unresolved} RFS lasciati al RO`:''}`
      :'';

    toast(
      'ATLAS ha generato la proposta',
      `${summary.added} assegnazioni proposte · ${summary.missing} ruoli scoperti · ${summary.cross} inserimenti fuori gruppo · ${summary.genderSwaps} riequilibri equipaggio${restText}${rfsText}.`,
  ...
  async function handleRfsPlacement(mode){
    ...
    const targetMonth=mode==='next'?nextMonthValue(state.month):state.month;
    if(mode==='current')removeAutoProposals();

    const placement=placeRfsEntitlements(entitlements,targetMonth);
    saveState();
    renderAll();

    if(mode==='current'){
      await generateAutomatic({
        __atlasGeneration:true,
        ...context.options,
        preserve:true,
        suppressRfsPrompt:true,
        rfsPlacement:placement
      });
      return;
    }

    finishAutomaticGeneration(context.summary,placement);
    const message=[
      `${placement.placed} RFS inseriti in ${monthLabel(targetMonth)}.`,
      placement.unresolved?`${placement.unresolved} non collocati e lasciati alla scelta del RO.`:'',
      placement.overdue?`${placement.overdue} risultano oltre il termine indicativo configurato.`:''
    ].filter(Boolean).join(' ');
  ...
  async function generateAutomatic(options=null){
    ...
    try{
      ...
      if(pendingRfs.length&&!suppressRfsPrompt){
        openRfsPlacementModal(pendingRfs,{
          options:{admin,resp,se,allowRo,autoRc},
          summary
        });
        return;
      }

      finishAutomaticGeneration(summary,inheritedRfsPlacement);
    }catch(err){
      console.error(err);
  ...
  function renderStaff(){
    ...
    let html='<thead><tr><th>Dipendente</th><th>Sesso</th><th>Gruppo</th><th>Ruoli</th><th>Responsabile</th><th>Sedi</th><th>Ore</th><th>RFS</th><th>Vincoli</th><th>Stato</th><th>Azioni</th></tr></thead><tbody>';
    ...
    list.forEach(e=>{
      ...
      html+=`<tr class="${e.attivo===false?'staff-inactive':''}">
        ...
        <td>${esc(limits)}</td>
        <td><span class="chip">${e.attivo===false?'Non attivo':'Attivo'}</span></td>
    ...
    $('#staffCount').textContent=`${state.employees.length} anagrafiche · ${state.employees.filter(e=>e.attivo!==false).length} attive`;
  }
