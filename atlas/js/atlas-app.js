    ...
    employees.forEach((e,i)=>{ const balance=hoursBalanceFor(e); html+=`<tr><td><span class="emp-index">${i+1}</span></td><td><div class="emp-name">${esc(employeeName(e))}</div><div class="emp-meta">${esc(employeeMeta(e))}</div></td><td><div class="group-cell"><span class="group-badge group-${e.turno.toLowerCase().replace(/\s+/g,'-')}">${esc(e.turno==='Amministrazione'?'AMM':e.turno)}</span><span class="hours-balance ${balance.cls}" title="${esc(balance.title)}">${esc(balance.label)}</span></div></td>`;
      dates.forEach(d=>{
        ...
        if(ended){
          ...
          html+=`<button class="cell-button ${items.length?'':'empty'} ${issue==='error'?'has-error':issue==='warning'?'has-warning':''}" data-employee="${e.id}" data-day="${day}" title="${esc(employeeName(e))} · ${day}">`;
          items.slice(0,3).forEach(a=>
            html+=`<span class="shift-tag ${tagClass(a)}" title="${sourceLabel(a)}${a.locked?' · bloccato':''}">${esc(normalizeCode(a))}${sourceLabel(a)==='AUTO'?'·':''}</span>`
          );
          if(items.length>3){
      ...
      [currentYear-1,currentYear].forEach(year=>{
        rfsEntitlementLedger(e.id,year)
          .filter(item=>
            ...
            item.due>=currentMonthStart
          )
          .forEach(item=>{
            const expiresThisMonth=
              item.due.slice(0,7)===state.month;
            ...
            out.push(validation(
              expiresThisMonth
                ...
                :'warning',
              expiresThisMonth
                ?'RFS obbligatorio non programmato'
                :'RFS da programmare',
