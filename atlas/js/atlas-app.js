import {
  ...
  DEFAULT_SETTINGS, DOW, FALLBACK_EMPLOYEES,
  MONTHS, ORDINARY_CREW_SLOTS, crewSlotsForDayShift
} from './config.js';
...
import { bootAuthentication, getServerAuthContext, rememberServerUrl } from './auth.js';
...
(() => {
  ...
  function normalizeEmployee(e){const weekly=Number(e.oreSettimanali)||null;return{...e,id:String(e.id||slug(`${e.cognome}-${e.nome}`)),turno:normalizeTurno(e.turno),sesso:normalizeSesso(e.sesso),autista:!!e.autista,capo:!!e.capo,soccorritore:!!e.soccorritore,l104:!!e.l104,avis:!!e.avis,congedo:!!e.congedo,attivo:e.attivo!==false,oreSettimanali:weekly,oreMensili:Number(e.oreMensili)||null,vacationAnnualHours:Number(e.vacationAnnualHours)||null,suppressedHolidayAnnualHours:Number(e.suppressedHolidayAnnualHours)||null,nightRestriction:['NO_NIGHT','ON_REQUEST'].includes(String(e.nightRestriction||'').toUpperCase())?String(e.nightRestriction).toUpperCase():'NONE',nightRestrictionFrom:String(e.nightRestrictionFrom||'').slice(0,10),nightRestrictionUntil:String(e.nightRestrictionUntil||'').slice(0,10),nightRestrictionNote:String(e.nightRestrictionNote||''),onCallNightRestricted:!!e.onCallNightRestricted,partTime:e.partTime===true||(weekly>0&&weekly<37.5),supplementaryConsent:!!e.supplementaryConsent,elasticClause:!!e.elasticClause,partTimeDays:String(e.partTimeDays||''),partTimeShifts:String(e.partTimeShifts||'')};}
  ...
  function tagClass(a){ if(a.category==='118')return a.shift==='M'?'tag-m':a.shift==='P'?'tag-p':a.shift==='PN'?'tag-pn':'tag-n'; if(a.category==='SE')return'tag-se'; if(a.category==='RESP')return'tag-resp'; if(a.category==='AM')return'tag-am'; if(a.category==='FORM')return'tag-form'; if(['RC','REST'].includes(a.category))return'tag-rc'; return'tag-absence'; }
  ...
  function ensurePostNightRest(employeeId,day,nightItem,{dirty=false}={}){
    ...
    const marker={
      ...
      category:'REST', type:'SM', code:'SM', hours:0, allDay:true,
      postNight:true, linkedNightId:nightItem.id,
      note:'Smonto/riposo obbligatorio dopo turno notturno - art. 28',
  ...
  function migrateAssignmentStore(){
    Object.keys(state.assignments).forEach(key=>{state.assignments[key]=(state.assignments[key]||[]).map(normalizeAssignmentItem);});
  }
  ...
  function weekHours(employeeId,date,extra=null){
    const start=mondayStart(date),end=new Date(start);end.setDate(end.getDate()+7);
    let total=workRowsForEmployee(employeeId).filter(r=>r.start<end&&r.end>start).reduce((s,r)=>s+Number(r.hours||0),0);
    if(extra){const t=assignmentTimes(extra.a,extra.day);if(t.timed&&t.start<end&&t.end>start)total+=Number(t.hours||0);}
    return round2(total);
  }
  ...
  function loadState(){
    try{
      ...
      state.assignments=data.assignments||{};migrateAssignmentStore();
  ...
  function appendAssignment(item){
    ...
    function commit(){
      ...
      toast('Assegnazione aggiunta',isNight118(candidate)?`${normalizeCode(candidate)} · SM inserito il giorno successivo`:normalizeCode(candidate));
    }
  }
  ...
  function addAbs(){const code=$('#fAbs').value,hours=numeric($('#fAbsHours').value),start=$('#fAbsStart').value,end=$('#fAbsEnd').value,e=currentEmployee(),meta=absenceMeta(code),bad=(code==='L104/92'&&!e.l104)||(code==='AVIS'&&!e.avis)||(code==='CONG'&&!e.congedo);if(meta.hourlyOnly&&!(start&&end))return toast('Permesso solo a ore',`${code} non può coprire l’intera giornata.`,'error');const item={category:'ABS',type:code,code,hours,start:start||'',end:end||'',allDay:!(start&&end),note:$('#fNote').value.trim(),eventDate:$('#fAbsEventDate')?.value||'',recoveryRequired:!!meta.requiresRecovery,recoveryDue:$('#fAbsRecoveryDue')?.value||'',ccnlRef:meta.ccnlRef};if(start&&end)item.nextDay=shiftWindow('CUSTOM',state.activeCell.day,start,end).nextDay;if(meta.autoRecoveryMonths&&!item.recoveryDue)item.recoveryDue=addMonthsKey(state.activeCell.day,meta.autoRecoveryMonths);if(bad)return confirmDialog('Causale non presente in matrice','Verifica autorizzazione',`La matrice non riporta ${code} per ${employeeName(e)}.`,()=>appendAssignment(item));appendAssignment(item);}
  ...
  function checkCandidate(e,day,item,{manual=false,allowRo=false}={}){
    ...
    if(isWorkingAssignment(item)&&e.partTime){
      const projected=weekHours(e.id,parseDateKey(day),{day,a:item}),contract=Number(e.oreSettimanali)||0,max=Math.min(state.settings.weeklyStandardHours||38,contract*1.4);
      if(contract&&projected>contract&&!e.supplementaryConsent)errors.push(`Superamento dell’orario part-time (${fmt(projected)}/${fmt(contract)} h) senza consenso al supplementare.`);
      if(contract&&projected>max)errors.push(`Lavoro part-time oltre il limite settimanale configurato (${fmt(max)} h).`);
    }
  ...
  function validateAll(){
    ...
    const currentYear=Number(state.month.slice(0,4)),holidays=holidayKeysForYear(currentYear,state.settings.patronHoliday),today=dateKey(new Date());
    state.employees.forEach(e=>{const used=new Set(),rcf=annualRows(e.id,currentYear).filter(r=>(r.a.code||r.a.type)==='RCF').sort((a,b)=>a.day.localeCompare(b.day));[...holidays].filter(day=>day.startsWith(state.month)).sort().forEach(day=>{const worked=getAssignments(e.id,day).some(isWorkingAssignment);if(!worked)return;const due=addDaysKey(day,state.settings.holidayRecoveryDays);const linked=rcf.find(r=>!used.has(r.a.id)&&(r.a.linkedEventDay===day||(!r.a.linkedEventDay&&r.day>day&&r.day<=due)));if(linked)used.add(linked.a.id);else out.push(validation(due<today?'error':'warning','Recupero festività da programmare',`${employeeName(e)} ha lavorato il ${formatDateIt(day)} e non risulta un RCF entro ${formatDateIt(due)}.`,e.id,day));});});
    monthDates().forEach(d=>{const day=dateKey(d),active=state.employees.filter(e=>e.attivo!==false).length,study=new Set(rowsForDay(day).filter(r=>(r.a.code||r.a.type)==='STUDIO').map(r=>r.employeeId)).size,bank=new Set(rowsForDay(day).filter(r=>(r.a.code||r.a.type)==='RCB').map(r=>r.employeeId)).size,studyMax=Math.max(1,Math.floor(active*.03)),bankMax=Math.max(1,Math.floor(active*.10));if(study>studyMax)out.push(validation('warning','Permessi studio oltre il 3%',`${formatDateIt(day)}: ${study} dipendenti, soglia indicativa ${studyMax}.`,null,day));if(bank>bankMax)out.push(validation('warning','Banca ore oltre il 10%',`${formatDateIt(day)}: ${bank} dipendenti, soglia ${bankMax}.`,null,day));});
  ...
  function renderSummary(){const year=Number(state.month.slice(0,4)),head=['Dipendente','Gruppo','Target','Ore','Saldo','Max settimana','Ferie','FS','PR36','Ore extra','M','P','N/P+N','Ruolo A','Ruolo C','Ruolo S','Weekend','SE','GRA','GRM','GRO','FORM','Assenze','AM','Auto'];let html=`<thead><tr>${head.map(h=>`<th>${h}</th>`).join('')}</tr></thead><tbody>`;state.employees.forEach(e=>{const s=employeeStats(e),target=targetHoursFor(e),delta=s.hours-target,cls=Math.abs(delta)<=2?'hours-ok':delta>0?'hours-pos':'hours-neg',weeks=monthDates().filter(d=>d.getDay()===1).map(d=>weekHours(e.id,d)),maxWeek=weeks.length?Math.max(...weeks):0;html+=`<tr><td><strong>${esc(employeeName(e))}</strong><div class="muted" style="font-size:9px;margin-top:2px">${esc(employeeMeta(e))}</div></td><td>${esc(e.turno==='Amministrazione'?'AMM':e.turno)}</td><td>${fmt(target)}</td><td class="hours-cell">${fmt(s.hours)}</td><td class="hours-cell ${cls}">${delta>0?'+':''}${fmt(delta)}</td><td>${fmt(maxWeek)}</td><td>${fmt(annualCodeHours(e.id,year,'F'))}</td><td>${fmt(annualCodeHours(e.id,year,'FS'))}</td><td>${fmt(annualCodeHours(e.id,year,'PR36'))}</td><td>${fmt(workRegimeHours(e.id,year))}</td><td>${s.M}</td><td>${s.P}</td><td>${s.N}</td><td>${s.roleA}</td><td>${s.roleC}</td><td>${s.roleS}</td><td>${s.weekends}</td><td>${s.se}</td><td>${s.gra}</td><td>${s.grm}</td><td>${s.gro}</td><td>${s.form}</td><td>${s.abs}</td><td>${s.am}</td><td>${s.auto}</td></tr>`;});html+='</tbody>';$('#summaryTable').innerHTML=html;}
  ...
  function togglePreGenMode(){
    ...
    if(!meta.requiresRecovery)$('#preGenRecoveryDue').value='';
  }
  ...
  function addPreGenerationDraft(){
    ...
    const recoveryDue=$('#preGenRecoveryDue').value;
    ...
    preGenerationDraft.forEach(entry=>{
      ...
      for(let d=new Date(from);d<=to;d.setDate(d.getDate()+1)){
        ...
        const item={
          ...
          recoveryDue:entry.recoveryDue||(absenceMeta(entry.code).autoRecoveryMonths?addMonthsKey(day,absenceMeta(entry.code).autoRecoveryMonths):''),
          preGeneration:true
        };
  ...
  function updateAbsenceRuleHint(){const code=$('#absCode').value,meta=absenceMeta(code),parts=[`${code} · ${meta.label}`,meta.ccnlRef];if(meta.hourlyOnly)parts.push('solo a ore');if(meta.requiresRecovery)parts.push('richiede recupero');if(meta.eventRequired)parts.push('richiede data evento');if(meta.referenceRequired)parts.push('richiede data di riferimento');if(meta.minHours)parts.push(`minimo ${meta.minHours} ore`);$('#absenceRuleHint').innerHTML=`<strong>${esc(parts.filter(Boolean).join(' · '))}</strong><br>Il record è protetto e non viene sovrascritto dal generatore.`;$('#absEventDate').disabled=!meta.eventRequired;$('#absReferenceDate').disabled=!meta.referenceRequired;$('#absRecoveryDue').disabled=!meta.requiresRecovery;if(!meta.eventRequired)$('#absEventDate').value='';if(!meta.referenceRequired)$('#absReferenceDate').value='';if(!meta.requiresRecovery)$('#absRecoveryDue').value='';toggleAbsenceMode();}
  ...
  function declareAbsence(){const employeeId=$('#absEmployee').value,from=parseFlexibleDate($('#absFrom').value),to=parseFlexibleDate($('#absTo').value),code=$('#absCode').value,meta=absenceMeta(code),mode=$('#absMode').value,hours=numeric($('#absHours').value),start=$('#absStart').value,end=$('#absEnd').value,note=$('#absNote').value.trim(),status=$('#absStatus').value,eventDate=$('#absEventDate').value,referenceDate=$('#absReferenceDate').value,recoveredHours=numeric($('#absRecoveredHours').value);if(!employeeId||!from||!to)return toast('Dati incompleti','Seleziona dipendente e intervallo.','error');if(!from.startsWith(state.month)||!to.startsWith(state.month))return toast('Mese non coerente',`Le date devono appartenere a ${monthLabel()}.`,'error');if(mode==='ORE'&&(!start||!end||hours<=0))return toast('Dati orari incompleti','Inserisci inizio, fine e ore.','error');if(meta.hourlyOnly&&mode!=='ORE')return toast('Causale solo a ore',`${code} non può essere registrato a giornata intera.`,'error');if(meta.eventRequired&&!eventDate)return toast('Data evento mancante',`${code} richiede la data dell’evento.`,'error');if(meta.referenceRequired&&!referenceDate)return toast('Data di riferimento mancante',`${code} richiede il record o evento da recuperare.`,'error');if(meta.minHours&&hours<meta.minHours)return toast('Blocco minimo non rispettato',`${code} richiede almeno ${meta.minHours} ore.`,'error');const a=parseDateKey(from),b=parseDateKey(to);if(b<a)return toast('Intervallo non valido','','error');let created=0,interrupted=0,skipped=0;for(let d=new Date(a);d<=b;d.setDate(d.getDate()+1)){const day=dateKey(d);let h=hours;if(mode==='GIORNATA'&&isWeekend(d)&&!['MAL','INF','MATR','LUTTO'].includes(code))h=0;if(['MAL','INF'].includes(code))interrupted+=interruptVacationForSickness(employeeId,day);const duplicate=getAssignments(employeeId,day).some(x=>isProtectedCalendarRecord(x)&&(x.code||x.type)===code&&!!x.allDay===(mode==='GIORNATA')&&(x.start||'')===(mode==='ORE'?start:'')&&(x.end||'')===(mode==='ORE'?end:''));if(duplicate){skipped++;continue;}let recoveryDue=$('#absRecoveryDue').value||'';if(meta.autoRecoveryMonths&&!recoveryDue)recoveryDue=addMonthsKey(day,meta.autoRecoveryMonths);const item={...manualBase(),category:meta.category,type:code,code,hours:h,allDay:mode==='GIORNATA',start:mode==='ORE'?start:'',end:mode==='ORE'?end:'',nextDay:mode==='ORE'?shiftWindow('CUSTOM',day,start,end).nextDay:false,note,status,locked:true,eventDate,linkedEventDay:referenceDate,recoveryRequired:!!meta.requiresRecovery,recoveryDue,recoveredHours,ccnlRef:meta.ccnlRef};appendTo(employeeId,day,item,{dirty:true,render:false});created++;}state.localDirty=true;saveState();renderAll();renderAbsenceBulkList();toast('Dichiarazione registrata e protetta',[`${created} record creati`,skipped?`${skipped} duplicati ignorati`:'',interrupted?`${interrupted} ferie interrotte da malattia/infortunio`:''].filter(Boolean).join(' · '),'success');}
  ...
  async function reloadSheets({replaceDb=true,silent=false}={}){
  ...
  async function syncCalendarGoogle(){
    ...
    try{
      ...
      toast(
        ...
        `${Number(data.rows||0)} record autenticati e sincronizzati.`,
        'success'
      );
    }catch(e){
      console.error(e);
      ...
      toast(
        'Scrittura autenticata non riuscita',
  ...
  function saveSettings(){
    ...
    state.settings={
      ...state.settings,
      targetHours:numeric($('#setTargetHours').value),
      minRest:numeric($('#setMinRest').value),
      seMin:numeric($('#setSeMin').value),
      seMax:numeric($('#setSeMax').value),
      seTarget:Math.min(numeric($('#setSeMax').value),2),
      respMin:numeric($('#setRespMin').value),
      respGoal:numeric($('#setRespGoal').value),weeklyStandardHours:numeric($('#setWeeklyStandard').value,38),weeklyMinHours:numeric($('#setWeeklyMin').value,28),weeklyMaxHours:numeric($('#setWeeklyMax').value,44),weeklyAverageMax:numeric($('#setWeeklyAverageMax').value,48),weeklyRestHours:numeric($('#setWeeklyRestHours').value,35),weeklyRestOccurrences14:numeric($('#setWeeklyRestOccurrences').value,2),annualOvertimeLimit:numeric($('#setOvertimeLimit').value,150),annualOvertimeExtended:numeric($('#setOvertimeExtended').value,250),vacationAnnualHours:numeric($('#setVacationAnnual').value,190),suppressedHolidayAnnualHours:numeric($('#setSuppressedHolidayAnnual').value,26),personalPermitAnnualHours:numeric($('#setPersonalPermitAnnual').value,36),holidayRecoveryDays:numeric($('#setHolidayRecoveryDays').value,30),bankHoursMinBlock:numeric($('#setBankHoursMinBlock').value,4),patronHoliday:$('#setPatronHoliday').value.trim(),enforceNoSplitDay:$('#setNoSplitDay').checked,
      matrixCsvUrl:'',
      databaseCsvUrl:'',
      appsScriptUrl:serverUrl,
      useABRotation:$('#setRotation').checked
    };
    ...
    toast(
      ...
      'Dopo una modifica del server esci ed effettua nuovamente l’accesso.',
      'success'
    );
  }
  ...
  function bindEvents(){
    ...
    $('#preGenMode').addEventListener('change',togglePreGenMode);$('#preGenCode').addEventListener('change',togglePreGenMode);
  ...
  async function init(){
    ...
    rememberServerUrl(state.settings.appsScriptUrl);
    restoreProtectedRecordsForCurrentMonth();
