...
} from './auth-protected.js?v=1.0.0-final-generator-fix-20260810';
import { initSidebarLayout } from './layout.js';
import {
  applyAccessProfile,
  initVolunteerCoverage,
  refreshWorkspace
} from './volunteer-coverage.js?v=1.0.0-SMART-STABILE-20260810-1815';
import {
  ...
  approveVolunteerProposalWithPlan, approveVolunteerProposalPatch,
  resetVolunteerChangesPatch
} from './google-sheet-service.js?v=1.0.0-SMART-STABILE-20260810-1815';
...
if(has118&&hasSE)out.push(validation('error','118 e Secondari nello stesso giorno',`${employeeName(e)} è assegnato sia al 118 sia ai Secondari.`,e.id,day));if(has118&&hasResp)out.push(validation('error','118 e giornata responsabile',`${employeeName(e)} ha 118 e responsabilità nello stesso giorno.`,e.id,day));if(hasSE&&hasResp)out.push(validation('error','Secondari e giornata responsabile',`${employeeName(e)} ha servizio operativo SE e responsabilità nello stesso giorno.`,e.id,day));if(items.some(isWorkingAssignment)&&hasAllAbs)out.push(validation('error','Turno durante assenza',`${employeeName(e)} presenta attività lavorativa durante un’assenza o un riposo per l’intera giornata.`,e.id,day));
  ...
  function volunteerConvertibleResponsibility(item){
    const code=String(
      item?.code||
      item?.type||
      ''
    ...
    return(
      item?.category==='RESP'&&
      ['GRA','GRM','GRO','RO'].includes(code)
    );
  }

  function volunteerMovableAssignment(item){
    ...
    if(
      String(item.coverage||'')
        .toUpperCase()
        .startsWith('VOLONTARI')
    ){
      return false;
    }

    return(
      ['118','OP','SE'].includes(item.category)||
      volunteerConvertibleResponsibility(item)
    );
  }
  ...
  function volunteerReasonPotentiallyRepairable(reason){
    const value=String(reason||'').toLowerCase();

    return[
      'sovrapposizione',
      'riposo di ',
      'l’assegnazione porterebbe la settimana',
      "l'assegnazione porterebbe la settimana",
      'superamento dell’orario part-time',
      "superamento dell'orario part-time",
      'lavoro part-time oltre il limite settimanale',
      'è già presente una sigla operativa',
      'secondari e 118',
      '118 e secondari',
      'giornata di responsabilità e 118',
      'sigla operativa non può essere aggiunta',
      'non può essere composta da due donne'
    ].some(token=>value.includes(token));
  }

  function volunteerAssignmentHours(item,day){
    try{
      return Number(
        assignmentTimes(item,day)?.hours||
        item?.hours||
        0
      );
    }catch{
  ...
  function volunteerDirectOptions(hole,role,item,reasonMap){
    ...
    if(existing.length){
      return[{
        ...
        warnings:[],
        score:-1000,
        text:`Ruolo già coperto da ${existing.map(row=>employeeName(row.employee)).join(', ')}`
      }];
    }
    ...
    return state.employees
      ...
      .map(employee=>{
        ...
        return{
          ...
          score:candidateScore(employee,day,item,null),
          text:`${employeeName(employee)} può coprire direttamente`
        };
      })
      .filter(Boolean)
      .sort((left,right)=>
        ...
        employeeName(left.cover).localeCompare(employeeName(right.cover),'it')
      )
      .slice(0,14);
  }

  function volunteerPotentialReliefRows(employee,targetDay){
    const targetDate=parseDateKey(targetDay);

    return rowsForEmployee(employee.id)
      .filter(row=>{
        if(
          !row.day.startsWith(state.month)||
          !volunteerMovableAssignment(row.a)
        ){
          return false;
        }

        if(
          volunteerConvertibleResponsibility(row.a)
        ){
          return row.day===targetDay;
        }

        // I vincoli modificabili che impediscono un nuovo turno
        // (sovrapposizione, 11 h di riposo e monte ore settimanale)
        // dipendono solo dalla stessa giornata o dalla stessa settimana.
        return(
          Math.abs(
            (parseDateKey(row.day)-targetDate)/86400000
          )<=6
        );
      })
      .sort((left,right)=>{
        const leftSame=left.day===targetDay?0:1;
        const rightSame=right.day===targetDay?0:1;
        ...
        const leftDistance=Math.abs(
          (parseDateKey(left.day)-targetDate)/86400000
        );

        const rightDistance=Math.abs(
          (parseDateKey(right.day)-targetDate)/86400000
        );

        return(
          leftDistance-rightDistance||
          Number(right.hours||0)-Number(left.hours||0)||
          String(left.day).localeCompare(String(right.day))
        );
      });
  }

  function volunteerReassignmentChains({
    fromEmployee,
    sourceDay,
    sourceItem,
    excludedIds,
    ...
    const MAX_DEPTH=1;
    const MAX_RESULTS=3;
    const allowRoForItem=
      ['118','OP'].includes(sourceItem?.category);

    if(!budget){
      budget={
        nodes:0,
        maxNodes:140,
        deadline:Date.now()+35,
        truncated:false
      };
    }
    ...
    const candidateItem={
      ...sourceItem,
      id:`VOL-MOVE-${sourceItem?.id||uid()}-${depth}`
    };

    const candidates=state.employees
      .filter(employee=>
        employeeActiveOn(employee,sourceDay)&&
        employee.turno!=='Amministrazione'&&
        employee.id!==fromEmployee.id&&
      ...
      .sort((left,right)=>
        left.score-right.score||
        employeeName(left.employee)
          .localeCompare(
            employeeName(right.employee),
            'it'
          )
    ...
    for(const candidate of candidates){
      ...
      if(
        ...
        Date.now()>Number(budget.deadline||Infinity)
      ){
        budget.truncated=true;
        break;
      }

      const directCheck=checkCandidate(
        candidate.employee,
        sourceDay,
        candidateItem,
        {
          manual:false,
          allowRo:allowRoForItem
        }
      );

      const step={
        fromEmployeeId:fromEmployee.id,
        fromEmployeeName:employeeName(fromEmployee),
        toEmployeeId:candidate.employee.id,
        toEmployeeName:employeeName(candidate.employee),
        sourceDay,
        sourceItemId:sourceItem.id,
        sourceCode:normalizeCode(sourceItem),
        sourceCategory:sourceItem.category
      };

      if(!directCheck.errors.length){
        results.push({
          cost:1,
          score:
            candidate.score+
      ...
      if(
        !directCheck.errors.every(
          volunteerReasonPotentiallyRepairable
        )
      ){
        continue;
      }
      ...
      for(const blocker of blockers){
        ...
        const attempted=
          withVolunteerAssignmentRemoved(
            candidate.employee.id,
            blocker.day,
            blocker.a.id,
            ()=>{
              ...
              if(afterRemoval.errors.length){
                return[];
              }

              if(
                volunteerConvertibleResponsibility(
                  blocker.a
                )&&
                blocker.day===sourceDay&&
                allowRoForItem
              ){
                return[{
                  cost:2,
                  score:
                    ...
                    310,
                  warnings:[
                    ...(afterRemoval.warnings||[]),
                    `${normalizeCode(blocker.a)} di ${employeeName(candidate.employee)} viene convertito in servizio operativo 118.`
                  ],
                  resources:[
                    candidate.employee.id
                  ],
                  ...
                  drops:[{
                    ...
                    day:blocker.day,
                    sourceItemId:blocker.a.id,
                    sourceCode:normalizeCode(blocker.a)
                  }]
                }];
              }
              ...
              const downstream=
                volunteerReassignmentChains({
                  fromEmployee:
                    candidate.employee,
                  sourceDay:blocker.day,
                  sourceItem:blocker.a,
                  excludedIds:
                    nextExcluded,
                  depth:depth+1,
                  budget
    ...
    results
      ...
      .forEach(chain=>{
        const signature=[
          ...(chain.steps||[])
            .map(step=>
              `${step.fromEmployeeId}:${step.sourceItemId}:${step.sourceDay}>${step.toEmployeeId}`
            ),
          ...(chain.drops||[])
            .map(drop=>
              `drop:${drop.employeeId}:${drop.sourceItemId}:${drop.day}`
            )
        ].join('|');

        if(!unique.has(signature)){
          unique.set(signature,chain);
        }
      });
  ...
  function volunteerChangeOptions(
    ...
    const budget={
      nodes:0,
      maxNodes:220,
      deadline:Date.now()+45,
      truncated:false
    };

    let hadRepairableCandidate=false;

    const candidates=state.employees
      .filter(employee=>
        volunteerEmployeeAllowed(
          employee,
          role
        )
      )
    ...
    for(const employee of candidates){
      ...
      if(!initialCheck.errors.length){
        continue;
      }

      if(
        !initialCheck.errors.every(
          volunteerReasonPotentiallyRepairable
        )
      ){
        initialCheck.errors.forEach(error=>
          reasonMap.set(
            error,
            (reasonMap.get(error)||0)+1
          )
      ...
      for(const sourceRow of reliefRows){
        budget.nodes++;
            ...
            ()=>{
              ...
              if(
                ...
                return[{
                  ...
                  cover:employee,
                  replacement:null,
                  sourceItem:sourceRow.a,
                  sourceDay:sourceRow.day,
                  chainSteps:[],
                  droppedResponsibilities:[{
                    ...
                    sourceItemId:
                      sourceRow.a.id,
                    sourceCode:
                      normalizeCode(sourceRow.a)
                  }],
                  warnings:[
                    ...(holeCheck.warnings||[]),
                    `${normalizeCode(sourceRow.a)} viene convertito in servizio 118 per coprire la richiesta volontari.`
                  ],
                  score:
                    candidateScore(
                      employee,
                  ...
                  text:
                    `${employeeName(employee)} è in ${normalizeCode(sourceRow.a)} e può passare al 118`
                }];
              }
              ...
              return chains.map(chain=>{
                ...
                return{
                  ...
                  text:
                    (chain.steps||[]).length>1
                      ?`${employeeName(employee)} può coprire ${role} con una catena di ${(chain.steps||[]).length} cambi`
                      :`${employeeName(employee)} può coprire ${role}; ${first?.toEmployeeName||'un collega'} prende ${normalizeCode(sourceRow.a)}`
                };
              });
            }
          );
    ...
    options
      ...
      .forEach(option=>{
        const signature=[
          ...
          ...(option.droppedResponsibilities||[])
            .map(drop=>
              `drop:${drop.employeeId}:${drop.sourceItemId}:${drop.day}`
            )
        ].join('|');
  ...
  function volunteerPlanOptions(roleAnalyses,limit=20){
    ...
    output.searchTruncated=
      truncated||
      unique.size>output.length;

    return output;
  }


  function volunteerHoursSnapshot(
    employee,
    adjustmentHours=0,
    contexts=[]
  ){
    ...
    const target=
      round2(
        targetHoursFor(employee)
      );

    const plannedBefore=
      round2(
        Number(stats.hours||0)
      );

    const projected=
      round2(
        plannedBefore+
        Number(adjustmentHours||0)
      );
    ...
    if(delta < -tolerance){
      status='missing';
      label=
        `Mancano ${fmt(Math.abs(delta))} h`;
    }else if(delta > tolerance){
      status='excess';
      label=
        `${fmt(delta)} h oltre target`;
    }
  ...
  function volunteerPlanHoursImpacts(plan){
    ...
    (plan?.plan||[])
      .forEach(option=>{
        ...
        touch(
          option.cover,
          holeHours,
          `Copre il ruolo ${option.role} della richiesta volontari`
        );
        ...
        if(option.type==='responsibility'){
          ...
          touch(
            option.cover,
            -sourceHours,
            `${normalizeCode(option.sourceItem)} convertito in servizio 118`
          );
        ...
        (option.chainSteps||[])
          .forEach(step=>{
            ...
            touch(
              fromEmployee,
              -hours,
              `${step.sourceCode} ceduto`
            );

            touch(
              toEmployee,
              hours,
              `${step.sourceCode} acquisito`
            );
          });
        ...
        (option.droppedResponsibilities||[])
          .forEach(drop=>{
            ...
            touch(
              ...
              `${drop.sourceCode} convertito in servizio operativo`
            );
          });
      });
  ...
  function volunteerPublicOperation(option){
    const base={
      ...
      sourceCode:
        option.sourceItem
          ?normalizeCode(option.sourceItem)
          :'',
      text:option.text,
  ...
  function volunteerPublicSolution(plan,index){
    ...
    const hoursImpacts=
      volunteerPlanHoursImpacts(plan);
    ...
    return{
      ...
      covered,
      hoursImpacts,
      coverageMissingHours:
        round2(
          hoursImpacts.reduce(
            (sum,snapshot)=>
              sum+
              Number(
                snapshot.missingHours||
                0
              ),
            0
          )
  ...
  function analyzeVolunteerProposalCompatibility(proposal){
    ...
    const roleAnalyses=roles.map(role=>{
      ...
      return{
        ...
        reasons,
        searchTruncated:
          changeSearch.truncated===true,
        potentiallyRepairable:
          changeSearch.hadRepairableCandidate===true
      };
    });

    const noOptions=roleAnalyses.filter(
      analysis=>!analysis.options.length
    );

    if(noOptions.length){
      const inconclusive=
        noOptions.some(analysis=>
          analysis.searchTruncated||
          analysis.potentiallyRepairable
        );
      ...
      if(inconclusive){
        return{
          ...
          tone:'warning',
          summary:
            'ATLAS ha esplorato cambi diretti, catene di sostituzione e spostamenti nel mese, ma non classifica questa richiesta come impossibile perché restano vincoli potenzialmente riorganizzabili.',
          ...
          changes:[],
          solutions:[],
          blockers:noOptions.flatMap(analysis=>{
            ...
            return analysis.reasons.length
              ...
              :[
                  prefix+
                  'nessuna soluzione conclusiva trovata.'
                ];
          }).slice(0,8),
          searchTruncated:
            noOptions.some(
      ...
      return{
        ...
        tone:'danger',
        summary:
          'La richiesta risulta realmente non copribile con il calendario attuale: non esistono risorse abilitate utilizzabili né cambi compatibili individuabili.',
        roles:roleAnalyses.map(analysis=>({
          role:analysis.role,
          directCount:analysis.directCount,
          changeCount:analysis.changeCount
        ...
        blockers:noOptions.flatMap(analysis=>{
          ...
          return analysis.reasons.length
            ...
            :[
                prefix+
                'nessuna risorsa compatibile trovata.'
              ];
        }).slice(0,8)
      };
    }
    ...
    if(!plans.length){
      ...
      if(inconclusive){
        return{
          ...
          tone:'warning',
          summary:
            'Le singole coperture esistono, ma la ricerca combinata non ha concluso con certezza una composizione completa. ATLAS non la dichiara impossibile.',
          roles:roleAnalyses.map(analysis=>({
            role:analysis.role,
            directCount:analysis.directCount,
            changeCount:analysis.changeCount
          ...
          solutions:[],
          blockers:[
            'Le soluzioni disponibili entrano in conflitto tra loro e la ricerca estesa ha raggiunto il limite di sicurezza.'
          ],
      ...
      return{
        ...
        tone:'danger',
        summary:
          'Le singole coperture esistono, ma non possono essere combinate senza utilizzare la stessa risorsa o lo stesso turno più volte.',
        roles:roleAnalyses.map(analysis=>({
          role:analysis.role,
          directCount:analysis.directCount,
          changeCount:analysis.changeCount
        ...
        solutions:[],
        blockers:[
          'Le risorse compatibili entrano in conflitto nella composizione completa della richiesta.'
        ]
      };
    }
    ...
    if(changes.length){
      return{
        status:'CHANGES',
        label:'Compatibile con cambi',
        tone:'warning',
        summary:
          solutions.length>1
            ?`ATLAS ha trovato ${solutions.length} soluzioni compatibili sull’intero calendario. La prima richiede ${best.cost} ${best.cost===1?'movimento':'movimenti'} di turno.`
            :best.cost===1
              ?'La richiesta è copribile con un solo adeguamento di turno.'
              :`La richiesta è copribile con una catena di ${best.cost} movimenti di turno.`,
    ...
    return{
      ...
      summary:
        solutions.length>1
          ?`ATLAS ha trovato ${solutions.length} soluzioni compatibili senza spostare altri turni.`
          :'La richiesta è compatibile con il calendario attuale senza spostare altri turni.',
      roles:best.operations,
      changes:[],
      solutions,
      blockers:[],
  ...
  function applyVolunteerProposalSolution(
    ...
    const suppliedSolution=
      options&&options.solution&&
      options.solution.signature===solutionSignature
        ?options.solution
        :null;

    // Nel flusso Approva e applica la soluzione è già stata calcolata
    // dalla pagina Buchi volontari. Evitiamo di rieseguire la ricerca
    // combinatoria completa: ogni singola operazione viene comunque
    // ricontrollata prima di modificare il calendario.
    const solution=suppliedSolution||(()=>{
      const fresh=
    ...
    const removeSource=(
      ...
      affected.add(
        `${employeeId}|${sourceDay}`
      );

      return found.item;
    };

    const addMovedAssignment=(
      employee,
      targetDay,
      sourceItem,
      description
      ...
      if(check.errors.length){
        throw new Error(
          `${employeeName(employee)} non è più compatibile con ${normalizeCode(sourceItem)} del ${targetDay}: ${check.errors.join(' ')}`
        );
      }
      ...
      affected.add(
        `${employee.id}|${targetDay}`
      );

      return moved;
    };

    const addHoleAssignment=(
      employee,
      role
    )=>{
      const holeItem=
      ...
      if(check.errors.length){
        throw new Error(
          `${employeeName(employee)} non è più compatibile con il buco ${role}: ${check.errors.join(' ')}`
        );
      }
    ...
    try{
      // 1) Catene di cambi. Prima si liberano tutte le caselle coinvolte,
      // poi si ricollocano i turni e infine la risorsa principale copre il buco.
      const chainOperations=
        solution.operations.filter(
          operation=>
      ...
      for(const operation of chainOperations){
        ...
        // Compatibilità con eventuali soluzioni create da versioni precedenti.
        if(
          !steps.length&&
          ...
          steps=[{
            ...
            toEmployeeId:
              operation.replacementEmployeeId,
            toEmployeeName:
              operation.replacementName,
            sourceDay:
              operation.sourceDay||day,
            sourceItemId:
              operation.sourceItemId,
            sourceCode:
        ...
        if(!steps.length){
          throw new Error(
            `Catena di cambio non disponibile per il ruolo ${operation.role}.`
          );
        }
        ...
        const stepRecords=
          steps.map(step=>{
            ...
            if(!fromEmployee||!toEmployee){
              throw new Error(
                'Una risorsa della catena di cambio non è più presente in anagrafica.'
              );
            }
            ...
            if(!found){
              throw new Error(
                `Il turno ${step.sourceCode||step.sourceItemId} di ${employeeName(fromEmployee)} non è più presente nel calendario.`
              );
            }
        ...
        const dropRecords=
          (operation.droppedResponsibilities||[])
            .map(drop=>{
              ...
              if(!employee||!found){
                throw new Error(
                  `La giornata ${drop.sourceCode||'di responsabilità'} da convertire non è più disponibile.`
                );
              }
              ...
              if(
                ...
                throw new Error(
                  `${normalizeCode(found.item)} non può essere convertito automaticamente in turno 118.`
                );
              }
        ...
        // Rimuove prima tutti i turni ceduti: in questo modo ogni
        // controllo successivo valuta lo scenario finale e non quello intermedio.
        stepRecords.forEach(record=>{
          const removed=
            removeSource(
              record.step.fromEmployeeId,
              record.step.sourceDay,
              record.step.sourceItemId
            );

          if(!removed){
            throw new Error(
              `Impossibile liberare ${record.step.sourceCode}.`
            );
          }
        });

        dropRecords.forEach(record=>{
          const removed=
            removeSource(
              record.drop.employeeId,
              record.drop.day,
              record.drop.sourceItemId,
              {
                removePostNight:false
              }
            );

          if(!removed){
            throw new Error(
              `Impossibile convertire ${record.drop.sourceCode}.`
            );
          }
        });
        ...
        stepRecords.forEach(record=>{
          addMovedAssignment(
            ...
            record.step.sourceDay,
            record.sourceItem,
            `Cambio per copertura volontari: ${employeeName(record.fromEmployee)} → ${employeeName(record.toEmployee)}`
          );
        });
      ...
      // 2) Giornate RO / GRA / GRM convertite direttamente in servizio 118.
      ...
      for(const operation of responsibilityOperations){
        const employee=
          state.employees.find(
            person=>
              person.id===
        ...
        if(
          ...
          throw new Error(
            `La giornata ${operation.sourceCode||'di responsabilità'} di ${employeeName(employee)} non è più disponibile per la conversione.`
          );
        }
      ...
      if(!options.deferUi){
        toast(
          'Piano volontari applicato',
          `${affectedCells.length} ${
            affectedCells.length===1
              ?'cella aggiornata'
              :'celle aggiornate'
          }. ATLAS ha applicato la soluzione selezionata mantenendo i controlli su riposi, qualifiche e monte ore.`,
          'success'
        );
      }
  ...
  const VOLUNTEER_JOURNAL_VERSION=1;

  function volunteerJournalStorageKey(){
    return`atlas118-volunteer-journal-v${VOLUNTEER_JOURNAL_VERSION}:${state.month}`;
  }

  function readVolunteerJournal(){
    try{
      const parsed=JSON.parse(
        localStorage.getItem(
          volunteerJournalStorageKey()
        )||'null'
      );
  ...
  function writeVolunteerJournal(journal){
    localStorage.setItem(
      volunteerJournalStorageKey(),
  ...
  function clearVolunteerJournal(){
    localStorage.removeItem(
      volunteerJournalStorageKey()
    );
  }

  function cellKeyFromParts(employeeId,day){
    return`${employeeId}|${day}`;
  }

  function rowsForAffectedCells(
    affectedCells,
    {
      proposalId='',
      approvedBy='',
    ...
    const marker=proposalId
      ?volunteerProposalMarker(proposalId)
      :'';
  ...
  function solutionChangeSummary(solution){
    return(solution?.operations||[])
      .filter(operation=>
        operation.mode!=='covered'
      )
      .map(operation=>{
        if(operation.mode==='direct'){
          return{
            ...
            role:operation.role||'',
            text:
              `${operation.coverName} copre ${operation.role}`
          };
        }

        if(operation.mode==='responsibility'){
          return{
            type:'CONVERSIONE',
            role:operation.role||'',
            text:
              `${operation.coverName}: ${operation.sourceCode} → turno 118 (${operation.role})`
          };
        }

        const chain=(operation.chainSteps||[])
          .map(step=>
  ...
  function prepareVolunteerJournalUpdate({
    beforeSnapshot,
    affectedCells,
    proposal,
    solution,
    ...
    const journal=readVolunteerJournal();

    (affectedCells||[]).forEach(cell=>{
      const key=cellKeyFromParts(
        cell.employeeId,
        cell.day
      );
    ...
    journal.entries.push({
      ...
      approvedAt,
      hole:volunteerProposalHole(proposal),
      changes:solutionChangeSummary(solution),
      hoursImpacts:clonePlan(
        solution?.hoursImpacts||[]
      )
    });
  ...
  function volunteerChangeHistory(){
    const journal=readVolunteerJournal();

    return{
      month:journal.month,
      count:journal.entries.length,
      entries:clonePlan(journal.entries)
    };
  }

  async function resetVolunteerChangesAndPersist(){
    const journal=readVolunteerJournal();
    ...
    const beforeReset=clonePlan(
      state.assignments||{}
    );
    const dirtyBefore=state.localDirty;

    const affectedCells=Object.keys(
      journal.cells||{}
    ).map(key=>{
      const split=key.lastIndexOf('|');
      return{
    ...
    Object.entries(journal.cells||{})
      .forEach(([key,items])=>{
        if(items===null){
          delete state.assignments[key];
        }else{
          state.assignments[key]=clonePlan(items);
        }
      });

    const rows=rowsForAffectedCells(
      affectedCells
    );

    updateSyncStatus(
      'Ripristino cambi volontari…',
      'sync'
    );

    try{
      const generationAt=String(
        state.lastAutoSummary?.at||''
      );

      const data=await resetVolunteerChangesPatch({
        url,
        token:auth.token,
        month:state.month,
        rows,
      ...
      clearVolunteerJournal();
      state.localDirty=false;
      saveState();
      renderAll();

      updateSyncStatus(
        'Cambi volontari ripristinati',
        'local'
      );
      ...
      state.assignments=beforeReset;
      state.localDirty=dirtyBefore;
      saveState();
      renderAll();
  ...
  async function approveVolunteerProposalAndPersist(
    ...
    const beforeSnapshot=clonePlan(
      state.assignments||{}
    );
    const dirtyBefore=state.localDirty;
    ...
    try{
      ...
      try{
        data=await approveVolunteerProposalPatch({
          url,
          token:auth.token,
          proposalId:proposal.id,
          reason,
        ...
        // il calendario viene comunque salvato, ma con il metodo completo.
        if(
          !/Azione non supportata|Azione non riconosciuta/i.test(
      ...
      return{
        ...
        approvedAt,
        affectedCells,
        history:volunteerChangeHistory()
      };
    }catch(error){
      state.assignments=beforeSnapshot;
      state.localDirty=dirtyBefore;
      saveState();
      renderAll();
  ...
  globalThis.ATLAS_VOLUNTEER_ANALYZER={
    ...
    changeHistory:
      volunteerChangeHistory,
    resetChanges:
      resetVolunteerChangesAndPersist,
    currentMonth:
      ()=>state.month
  };
  ...
  async function syncCalendarGoogle(){
    ...
    try{
      ...
      try{
        await refreshWorkspace({blocking:false});
      }catch(error){
        console.warn(
