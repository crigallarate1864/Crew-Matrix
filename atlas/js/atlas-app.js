...
(() => {
  ...
  function saveStaffRecord(){const oldId=state.staffEditingId,id=oldId||nextEmployeeId(),cognome=$('#staffSurname').value.trim(),nome=$('#staffName').value.trim(),employmentEndType=$('#staffEmploymentEndType').value,employmentEndDate=$('#staffEmploymentEndDate').value;if(!cognome||!nome)return toast('Dati incompleti','Cognome e nome sono obbligatori.','error');if(employmentEndType==='DATE'&&!employmentEndDate)return toast('Data termine mancante','Seleziona la data di termine oppure scegli Fine indeterminata.','error');const rec=normalizeEmployee({id,cognome,nome,sesso:$('#staffSex').value,turno:$('#staffTurn').value,autista:$('#staffDriver').checked,capo:$('#staffLeader').checked,soccorritore:$('#staffRescuer').checked,l104:$('#staff104').checked,avis:$('#staffAvis').checked,congedo:$('#staffParental').checked,responsabile:$('#staffResponsible').value,sedeSolo:$('#staffSites').value==='G'?'G':'',oreSettimanali:numeric($('#staffWeeklyHours').value,0)||null,oreMensili:numeric($('#staffMonthlyHours').value,0)||null,vacationAnnualHours:numeric($('#staffVacationHours').value,0)||null,suppressedHolidayAnnualHours:numeric($('#staffSuppressedHours').value,0)||null,nightRestriction:$('#staffNightRestriction').value,nightRestrictionFrom:$('#staffNightFrom').value,nightRestrictionUntil:$('#staffNightUntil').value,nightRestrictionNote:$('#staffNightNote').value.trim(),onCallNightRestricted:$('#staffOnCallNightRestricted').checked,partTime:$('#staffPartTime').checked,supplementaryConsent:$('#staffSupplementaryConsent').checked,elasticClause:$('#staffElasticClause').checked,partTimeDays:$('#staffPartTimeDays').value.trim(),partTimeShifts:$('#staffPartTimeShifts').value.trim(),employmentEndType,employmentEndDate:employmentEndType==='DATE'?employmentEndDate:'',attivo:$('#staffActive').checked});if(oldId){const idx=state.employees.findIndex(e=>e.id===oldId);state.employees[idx]=rec;}else state.employees.push(rec);state.employees.sort((a,b)=>a.turno.localeCompare(b.turno)||a.cognome.localeCompare(b.cognome));state.localDirty=true;saveState();renderAll();closeModal('staffModal');toast(oldId?'Anagrafica aggiornata':'Dipendente aggiunto',employeeName(rec));}
