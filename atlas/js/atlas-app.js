...
(() => {
  ...
  function rfsRecoveryWindow(sourceDay){
    const sourceMonth=String(sourceDay).slice(0,7);
    const nextMonth=nextMonthValue(sourceMonth);
    return{
      start:addDaysKey(sourceDay,1),
      end:endOfMonthKey(nextMonth),
      nextMonth
    };
  }
      ...
      rfsEntitlementLedger(e.id,currentYear)
        ...
        .forEach(item=>{
          out.push(validation(
            ...
            'RFS da programmare',
            `${employeeName(e)} ha maturato un RFS lavorando il ${formatDateIt(item.sourceDay)}. È utilizzabile fino al ${formatDateIt(item.due)}, ultimo giorno del mese successivo.`,
            e.id,
            item.sourceDay
          ));
        });
  ...
  function createRfsItem(employee,entitlement,targetMonth){
    ...
    return{
      ...
      note:`RFS maturato per festività lavorata del ${formatDateIt(entitlement.sourceDay)} · utilizzabile fino al ${formatDateIt(entitlement.due)}.`,
      createdAt:now,
      updatedAt:now
    };
  }
  ...
  function finishAutomaticGeneration(summary,rfsPlacement=null){
    ...
    const rfsText=rfsPlacement
      ?` · ${rfsPlacement.placed} RFS collocati senza scoperture${rfsPlacement.replacements?` · ${rfsPlacement.replacements} turni sostituiti e coperti`:''}${rfsPlacement.unresolved?` · ${rfsPlacement.unresolved} RFS lasciati al RO`:''}`
      :'';
