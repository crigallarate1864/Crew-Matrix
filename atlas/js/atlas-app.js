...
(() => {
  ...
  function appendAutomaticRest(employee,day,code,note,extra={}){
    ...
    addAuto(employee,day,{
      ...
      hours:0,
      allDay:true,
      ccnlRef:code==='RCF'?'Art. 29':code==='RCD'?'Art. 27':'Art. 28',
      note,
      ...extra
    });
    return true;
  ...
  function scheduleAutomaticCompensatoryRests(){
    ...
    state.employees.filter(e=>e.attivo!==false).forEach(employee=>{
      ...
      rows.filter(r=>isWorkingAssignment(r.a)&&holidays.has(r.day)).forEach(source=>{
        const exists=directCurrentRowsForEmployee(employee.id).some(r=>r.a.category==='RC'&&r.a.code==='RCF'&&(r.a.linkedRecordId===source.a.id||r.a.linkedEventDay===source.day));
        if(exists)return;
        const due=addDaysKey(source.day,Number(state.settings.holidayRecoveryDays||30));
        const candidate=automaticRestCandidate(employee.id,addDaysKey(source.day,1),due,{preferSunday:false});
        if(candidate&&appendAutomaticRest(employee,candidate.day,'RCF',`Recupero automatico della festività lavorata del ${formatDateIt(source.day)}.`,{
  ...
  async function generateAutomatic(){
    ...
    const preserve=$('#autoPreserve')?.checked!==false,
      admin=$('#autoAdmin')?.checked!==false,
      resp=$('#autoResp')?.checked!==false,
      se=$('#autoSe')?.checked!==false,
      allowRo=$('#autoRo')?.checked===true,
      autoRc=$('#autoRc')?.checked===true;
    ...
    await yieldUi();
    let added=0,r={added:0,missing:0,cross:0},restSummary={added:0,weekly:0,holiday:0,derogation:0,unresolved:0};
    try{
      if(!preserve)state.assignments={};else removeAutoProposals();
      updateGeneration(8,'Indicizzazione disponibilità e vincoli…');
      beginAutoCache();
      await yieldUi();
      if(admin){updateGeneration(15,'Pianificazione amministrazione…');added+=scheduleAdmin();await yieldUi();}
      if(resp){updateGeneration(24,'Pianificazione giornate responsabili…');added+=scheduleFixedResponsibles();await yieldUi();}
      if(se){updateGeneration(34,'Pianificazione Secondari…');added+=scheduleSecondari();await yieldUi();}
      updateGeneration(42,'Composizione equipaggi 118…');
      r=await schedule118({allowRo,onProgress:(done,total)=>updateGeneration(42+(done/total)*30,`Composizione equipaggi 118 · giorno ${done}/${total}`)});
      added+=r.added;
      updateGeneration(74,'Riequilibrio della composizione equipaggi…');
      ...
      const restText=autoRc?` · ${restSummary.added} RC automatici${restSummary.unresolved?` · ${restSummary.unresolved} recuperi da verificare`:''}`:'';
      toast('ATLAS ha generato la proposta',`${added} assegnazioni proposte · ${r.missing} ruoli scoperti · ${r.cross} inserimenti fuori gruppo · ${genderSwaps} riequilibri equipaggio${restText}.`,r.missing?'error':'success');
  ...
  function saveSettings(){
    ...
    toast(
      ...
      'Il collegamento al server ATLAS resta configurato automaticamente.',
      'success'
    );
  }
  ...
  function bindEvents(){
    ...
    document.addEventListener('keydown',e=>{if(e.key==='Escape')$$('.modal-backdrop.open').forEach(m=>closeModal(m.id));});window.addEventListener('beforeprint',prepareA3Print);window.addEventListener('afterprint',restoreAfterPrint);
  }


  function applyTemporaryAccessUi(auth){
    if(!auth?.temporary)return;

    const serverActions=[
      ['syncBtn','Solo locale'],
      ['reloadSheetBtn','Non disponibile'],
      ['syncStaffBtn','Non disponibile'],
      ['importEmployeesBtn','Non disponibile']
    ];

    serverActions.forEach(([id,title])=>{
      const button=$('#'+id);
      if(!button)return;
      button.disabled=true;
      button.dataset.serverAction='true';
      button.title=title+' in modalità temporanea';
    });

    if($('#syncBtnText'))$('#syncBtnText').textContent='Solo locale';
    updateSyncStatus('Modalità temporanea · solo browser','sync');
  }
  ...
  async function init(){
    ...
    initSidebarLayout();
    renderAll();
    applyTemporaryAccessUi(auth);

    if(auth.temporary){
      toast(
        'Accesso temporaneo attivo',
        'Stai usando soltanto i dati salvati in questo browser. Google Sheet non viene letto né modificato.',
        'info'
      );
      return;
    }
