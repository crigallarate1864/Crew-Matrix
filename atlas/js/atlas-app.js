...
if(has118&&hasSE)out.push(validation('error','118 e Secondari nello stesso giorno',`${employeeName(e)} è assegnato sia al 118 sia ai Secondari.`,e.id,day));if(has118&&hasResp)out.push(validation('error','118 e giornata responsabile',`${employeeName(e)} ha 118 e responsabilità nello stesso giorno.`,e.id,day));if(hasSE&&hasResp)out.push(validation('error','Secondari e giornata responsabile',`${employeeName(e)} ha servizio operativo SE e responsabilità nello stesso giorno.`,e.id,day));if(items.some(isWorkingAssignment)&&hasAllAbs)out.push(validation('error','Turno durante assenza',`${employeeName(e)} presenta attività lavorativa durante un’assenza o un riposo per l’intera giornata.`,e.id,day));
  ...
  // Preferenza SOFT: nella stessa settimana, laddove i vincoli hard lo permettono,
  // privilegia la continuità Gallarate/Somma per limitare gli spostamenti.
  function weeklySiteContinuityAdjustment(employeeId,day,item){
    ...
    const rows=rowsForEmployee(employeeId).filter(row=>
      row.day>=weekStart&&row.day<=weekEnd&&
      row.a?.category==='118'&&['G','S'].includes(row.a.site)
    );
    if(!rows.length)return 0;

    const same=rows.filter(row=>row.a.site===item.site).length;
    const other=rows.length-same;
    let adjustment=0;

    // Premia una settimana già orientata sulla stessa sede e penalizza gli zig-zag.
    if(same)adjustment-=Math.min(30,8+same*7);
    if(other)adjustment+=Math.min(48,12+other*11);

    const previous=rows
      .filter(row=>row.day<day)
      .sort((a,b)=>b.day.localeCompare(a.day))[0];
    if(previous){
      adjustment+=previous.a.site===item.site?-14:22;
    }
  ...
  function candidateScore(e,day,item,preferred,{preferMale=false,preserveMale=false}={}){
    ...
    score+=weeklySiteContinuityAdjustment(e.id,day,item);
