import {
  ...
  DATABASE_CSV_URL, DEFAULT_SETTINGS, DOW, FALLBACK_EMPLOYEES,
...
import { bootAuthentication } from './auth.js';
import {
  isValidAppsScriptUrl, loadPublishedSheets,
...
(() => {
  ...
  async function reloadSheets({replaceDb=true,silent=false}={}){
    updateSyncStatus('Caricamento Google Sheet…','sync');
    let matrixCount=0,dbCount=0;
    try{
      const {matrixText,databaseText}=await loadPublishedSheets({
        matrixUrl:state.settings.matrixCsvUrl,
        databaseUrl:state.settings.databaseCsvUrl
      });
      ...
      saveState();
      renderAll();
      updateSyncStatus(`Matrice ${matrixCount} · Database ${dbCount}`,'local');
      if(!silent)toast('Dati caricati',`${matrixCount} dipendenti · ${dbCount} record database.`);
    }catch(e){
      console.error(e);
      updateSyncStatus('Sheet non raggiungibile · uso dati locali','error');
      if(!silent)toast('Caricamento non riuscito',`${e.message}. Restano disponibili i dati locali/fallback.`,'error');
    }
  }
  ...
  async function syncEmployeesGoogle(){
    ...
    if(!url){
      exportEmployeesCsv();
      return toast('Matrice CSV esportata','Per scrivere direttamente nel foglio Matrice configura la Web App Apps Script.');
    }
    updateSyncStatus('Salvataggio anagrafiche…','sync');
    try{
      ...
      toast('Matrice aggiornata',`${data.employees||state.employees.length} anagrafiche salvate.`);
    }catch(e){
      updateSyncStatus('Errore sincronizzazione','error');
      toast('Salvataggio anagrafiche fallito',e.message,'error');
    }
  }
