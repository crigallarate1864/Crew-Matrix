import {
  APP_VERSION, AUTH_SESSION_KEY, AUTH_USERS, COMPATIBLE_STORAGE_VERSIONS,
  CREW_SLOTS, DATABASE_CSV_URL, DEFAULT_SETTINGS, DOW, FALLBACK_EMPLOYEES,
  MATRIX_CSV_URL, MONTHS, ORDINARY_CREW_SLOTS, crewSlotsForDayShift
} from './config.js';
...
(() => {
  ...
  function normalizeCode(a){ if(a.category==='118'){const base=a.shift==='PN'?'PN':a.shift;if(a.site==='G')return`${base}G${a.machine}${a.role}`;if(a.site==='SU')return`${base}SU${a.role}`;return`${base}S${a.role}`;} return a.code||a.type||''; }
  ...
  function isNight118(a){ return a?.category==='118' && ['N','PN'].includes(a.shift); }
  function isWorkingAssignment(a){
    return ['118','SE','RESP','AM','FORM','CUSTOM'].includes(a?.category);
  }
  function previousDayHasNight(employeeId,day){
    const previous=adjacentDayKey(day,-1);
    return getAssignments(employeeId,previous).some(isNight118);
  }
  function nextDayWorkAfterNight(employeeId,day){
    const next=adjacentDayKey(day,1);
    return getAssignments(employeeId,next).filter(isWorkingAssignment);
  }
  function ensurePostNightRest(employeeId,day,nightItem,{dirty=false}={}){
    if(!isNight118(nightItem))return false;
    const next=adjacentDayKey(day,1);
    if(!next.startsWith(state.month))return false;
    const existing=getAssignments(employeeId,next);
    if(existing.length)return false;
    const marker={
      id:uid(), origin:nightItem.origin||'AUTOMATICA', locked:!!nightItem.locked,
      status:nightItem.status||'PROPOSTO', coverage:'ORDINARIA',
      category:'RC', type:'RC', code:'RC', hours:0, allDay:true,
      postNight:true, linkedNightId:nightItem.id,
      note:'Riposo obbligatorio dopo turno notturno',
      createdAt:new Date().toISOString(), updatedAt:new Date().toISOString()
    };
    state.assignments[assignmentKey(employeeId,next)]=[marker];
    if(dirty)state.localDirty=true;
    return true;
  }
  function removeLinkedPostNightRest(employeeId,nightDay,nightItem){
    if(!isNight118(nightItem))return;
    const next=adjacentDayKey(nightDay,1),key=assignmentKey(employeeId,next);
    const items=getAssignments(employeeId,next);
    const keep=items.filter(a=>!(a.postNight && a.linkedNightId===nightItem.id));
  ...
  function ensureAllPostNightRests(){
    Object.entries(state.assignments).forEach(([key,items])=>{
      const split=key.lastIndexOf('|'),employeeId=key.slice(0,split),day=key.slice(split+1);
      items.forEach(a=>ensurePostNightRest(employeeId,day,a,{dirty:false}));
    });
  }
  ...
  function isProtectedCalendarRecord(a){
    return !!a && (a.category==='ABS' || (a.category==='RC' && !a.postNight));
  }
  ...
  function loadState(){
    try{
      ...
      state.localDirty=!!data.localDirty;
      state.lastAutoSummary=data.lastAutoSummary||null;
  ...
  function beginAutoCache(){const rows=buildAssignmentRows(),idx=indexRows(rows);state._autoRowsByEmployee=idx.byEmployee;state._autoShiftRows=idx.byShift;}
  function endAutoCache(){delete state._autoRowsByEmployee;delete state._autoShiftRows;}
  ...
  function renderCalendar(){
    const dates=monthDates(),employees=filteredEmployees(); $('#calendarTitle').textContent=monthLabel(); $('#calendarSubtitle').textContent=`${employees.length} dipendenti visualizzati · saldo: − ore mancanti, + ore in eccesso`;
  ...
  function openAssignment(employeeId,day){
    const e=state.employees.find(x=>x.id===employeeId);if(!e)return;
  ...
  function renderAssignmentForm(type){
    ...
    if(type==='118'){
      ...
      const update=()=>{
        ...
        const item={category:'118',shift,site,machine:(site==='G'&&!['N','PN'].includes(shift))?$('#fMachine').value:'3',role:$('#fRole').value};
      ...
      ['fShift','fSite','fMachine','fRole'].forEach(id=>$('#'+id).addEventListener('change',update));
      update();
  ...
  function appendAssignment(item){
    ...
    function commit(){
      ...
      toast('Assegnazione aggiunta',isNight118(candidate)?`${normalizeCode(candidate)} · RC inserito il giorno successivo`:normalizeCode(candidate));
    }
  }
  ...
  function checkCandidate(e,day,item,{manual=false,allowRo=false}={}){
    ...
    if(isWorkingAssignment(item) && previousDayHasNight(e.id,day)){
      errors.push('Il giorno successivo a un turno notturno è riservato al riposo: non sono consentite attività lavorative.');
    }
    if(isNight118(item)){
      const nextWork=nextDayWorkAfterNight(e.id,day);
      if(nextWork.length){
        errors.push(`Il turno notturno richiede riposo per l’intera giornata successiva, ma risultano già: ${nextWork.map(normalizeCode).join(', ')}.`);
      }
    }
  ...
  function validateAll(){
    ...
    state.employees.forEach(e=>{
      ...
      monthDates().forEach(d=>{const day=dateKey(d),items=getAssignments(e.id,day),has118=items.some(a=>a.category==='118'),hasSE=items.some(a=>a.category==='SE'),hasResp=items.some(a=>a.category==='RESP'),hasAllAbs=items.some(isAllDayBlock);
        if(previousDayHasNight(e.id,day)){
          const work=items.filter(isWorkingAssignment);
          if(work.length)out.push(validation('error','Riposo post-notte non rispettato',`${employeeName(e)} ha lavorato di notte il giorno precedente e risulta assegnato a ${work.map(normalizeCode).join(', ')}. La giornata deve essere di riposo.`,e.id,day));
  ...
  function renderKpis(){const errors=state.validations.filter(v=>v.severity==='error').length,warnings=state.validations.filter(v=>v.severity==='warning'&&v.title!=='Ruolo 118 scoperto').length,uncovered=state.validations.filter(v=>v.title==='Ruolo 118 scoperto').length,seOk=workdays().filter(d=>{const day=dateKey(d),c=rowsForDay(day).filter(r=>r.a.category==='SE').length;return c>=state.settings.seTarget;}).length;$('#kpiErrors').textContent=errors;$('#kpiWarnings').textContent=warnings;$('#kpiUncovered').textContent=uncovered;$('#kpiSeOk').textContent=seOk;const required=monthDates().reduce((n,d)=>n+['M','P','N'].reduce((sum,s)=>sum+(state.requirements[`${dateKey(d)}|${s}`]==='required'?crewSlotsForDayShift(d,s).length:0),0),0),filled=allAssignmentRows().filter(r=>r.a.category==='118').length,pct=Math.min(100,Math.round(filled/Math.max(1,required)*100));$('#completionText').textContent=`${pct}%`;$('#completionBar').style.width=`${pct}%`;const chip=$('#monthStatusChip');chip.textContent=!filled?'Bozza':errors?'Da correggere':uncovered?'Incompleto':warnings?'Da verificare':'Valido';chip.style.color=!filled?'var(--muted)':errors?'#ffb0bd':warnings||uncovered?'#ffd978':'#84edbd';}

  function employeeEligibleRole(e,role){return role==='A'?e.autista:role==='C'?e.capo:e.soccorritore;}
  function candidateScore(e,day,item,preferred,{preferMale=false,preserveMale=false}={}){
  ...
  function chooseCandidate(day,item,{preferred=null,allowRo=false,pool=null,preferMale=false,preserveMale=false,requireMale=false}={}){let list=(pool||state.employees).filter(e=>e.attivo!==false&&e.turno!=='Amministrazione'&&(item.category!=='118'||employeeEligibleRole(e,item.role)));if(requireMale)list=list.filter(isMale);list=list.filter(e=>checkCandidate(e,day,item,{allowRo}).errors.length===0);if(!allowRo)list=list.filter(e=>e.turno!=='RO');const ranked=list.map(e=>({e,score:candidateScore(e,day,item,preferred,{preferMale,preserveMale})})).sort((a,b)=>a.score-b.score);return ranked[0]?.e||null;}
  ...
  function preGenCodeLabel(code){
    const labels={
      F:'Ferie',PERM:'Permesso','L104/92':'Permesso 104',AVIS:'Permesso AVIS',
      CONG:'Congedo parentale',MAL:'Malattia',INF:'Infortunio',
      A:'Assenza festività',RC:'Riposo compensativo'
    };
  ...
  function addPreGenerationDraft(){
    ...
    preGenerationDraft.push({
      id:uid(),employeeId,from,to,code,mode,start,end,hours,status,note
    });
    $('#preGenNote').value='';
  ...
  function commitPreGenerationDraft(){
    ...
    preGenerationDraft.forEach(entry=>{
      ...
      for(let d=new Date(from);d<=to;d.setDate(d.getDate()+1)){
        ...
        const item={
          ...manualBase(),
          category:entry.code==='RC'||entry.code==='A'?'RC':'ABS',
  ...
  async function schedule118({allowRo=false,onProgress=null}={}){let added=0,missing=0,cross=0;const dates=monthDates();for(let i=0;i<dates.length;i++){const d=dates[i],day=dateKey(d);for(const shift of ['M','P','N']){if(state.requirements[`${day}|${shift}`]!=='required')continue;const targetGroup=chooseShiftGroup(d,shift),other=targetGroup==='A'?'B':'A',activeCrews=[...new Set(crewSlotsForDayShift(d,shift).map(s=>s.crew))];for(const slot of crewSlotsForDayShift(d,shift)){if(currentSlotOccupied(day,shift,slot))continue;const item={category:'118',shift,site:slot.site,machine:slot.machine,role:slot.role,plannedGroup:targetGroup};const currentRows=shiftRows(day,shift),crewRows=currentRows.filter(r=>crewKey(r.a)===slot.crew),hasMale=crewRows.some(r=>isMale(r.employee)),requireMale=slot.crew==='G2'&&crewRows.length>=1&&!hasMale,otherCrewNeedsMale=activeCrews.some(crew=>crew!==slot.crew&&!currentRows.filter(r=>crewKey(r.a)===crew).some(r=>isMale(r.employee))),preserveMale=hasMale&&otherCrewNeedsMale;let e=chooseCandidate(day,item,{preferred:targetGroup,allowRo,pool:state.employees.filter(x=>x.turno===targetGroup||x.turno==='Libera'),preferMale:!hasMale,preserveMale,requireMale});if(!e){e=chooseCandidate(day,item,{preferred:targetGroup,allowRo,pool:state.employees.filter(x=>x.turno===other),preferMale:!hasMale,preserveMale,requireMale});if(e){item.crossGroup=true;cross++;}}if(!e&&allowRo)e=chooseCandidate(day,item,{preferred:targetGroup,allowRo:true,pool:state.employees.filter(x=>x.turno==='RO'),preferMale:!hasMale,preserveMale,requireMale});if(!e){missing++;continue;}addAuto(e,day,item);added++;}}if(i%2===1||i===dates.length-1){if(onProgress)onProgress(i+1,dates.length);await yieldUi();}}return{added,missing,cross};}

  async function generateAutomatic(){if(state.generating)return toast('Generazione già in corso','Attendi il completamento della proposta.','info');const preserve=$('#autoPreserve')?.checked!==false,admin=$('#autoAdmin')?.checked!==false,resp=$('#autoResp')?.checked!==false,se=$('#autoSe')?.checked!==false,allowRo=$('#autoRo')?.checked===true;closeModal('autoModal');setGenerationUi(true,2,'Avvio del motore di turnazione…');await yieldUi();let added=0,r={added:0,missing:0,cross:0};try{if(!preserve)state.assignments={};else removeAutoProposals();updateGeneration(8,'Indicizzazione disponibilità e vincoli…');beginAutoCache();await yieldUi();if(admin){updateGeneration(15,'Pianificazione amministrazione…');added+=scheduleAdmin();await yieldUi();}if(resp){updateGeneration(24,'Pianificazione giornate responsabili…');added+=scheduleFixedResponsibles();await yieldUi();}if(se){updateGeneration(34,'Pianificazione Secondari…');added+=scheduleSecondari();await yieldUi();}updateGeneration(42,'Composizione equipaggi 118…');r=await schedule118({allowRo,onProgress:(done,total)=>updateGeneration(42+(done/total)*34,`Composizione equipaggi 118 · giorno ${done}/${total}`)});added+=r.added;endAutoCache();updateGeneration(78,'Riequilibrio della composizione equipaggi…');const genderSwaps=rebalanceCrewGender();await yieldUi();updateGeneration(84,'Salvataggio della proposta…');state.localDirty=true;state.lastAutoSummary={at:new Date().toISOString(),added,missing:r.missing,cross:r.cross,genderSwaps};saveState();await yieldUi();updateGeneration(90,'Verifica riposi, ruoli e coperture…');renderAll();await yieldUi();updateGeneration(100,'Proposta pronta');toast('ATLAS ha generato la proposta',`${added} assegnazioni proposte · ${r.missing} ruoli scoperti · ${r.cross} inserimenti fuori gruppo · ${genderSwaps} riequilibri equipaggio.`,r.missing?'error':'success');switchView('anomaliesView');}catch(err){console.error(err);toast('Generazione non completata',err.message||'Errore imprevisto','error');}finally{endAutoCache();setGenerationUi(false);}}

  function openAutoModal(){if(state.generating)return toast('Generazione già in corso','Attendi il completamento.','info');const req=Object.values(state.requirements).filter(v=>v==='required').length;const protectedCount=existingProtectedForRecap().length;$('#autoInfo').innerHTML=`Sono state verificate <strong>${protectedCount} indisponibilità protette</strong> per ${monthLabel()}. <strong>ATLAS</strong> mantiene separati i gruppi <strong>A/B</strong>; le <strong>Libere</strong> partecipano come risorse neutrali secondo le abilitazioni. Alterna i ruoli A/C/S, distribuisce gli uomini tra gli equipaggi che ne sono privi e applica la regola rigida: <strong>la macchina a 2 non può avere due donne</strong>. La generazione ordinaria comprende <strong>Gallarate e Somma</strong>; <strong>Sumirago è inserita solo manualmente al bisogno</strong>. Il sabato mattina pianifica <strong>Gallarate macchina a 2, Gallarate macchina a 3 e Somma</strong>; il sabato pomeriggio pianifica <strong>Gallarate macchina a 2 e Somma</strong>; di notte usa soltanto equipaggi a 3. Raschi resta in <strong>GRS</strong> salvo necessità MGSE. Saranno completate ${req} fasce richieste.`;openModal('autoModal');}
  ...
  function applyDbRecordsToMonth({replace=true}={}){if(replace){state.assignments={};state.requirements={};initRequirements();}state.dbRecords.forEach(r=>{if(r.requirement){if(r.day.startsWith(state.month))state.requirements[`${r.day}|${r.shift}`]='required';return;}if(!r.day.startsWith(state.month))return;let emp=state.employees.find(e=>e.id===r.employeeId);if(!emp){const simple=slug(r.employeeId);emp=state.employees.find(e=>slug(e.id)===simple);}if(emp)appendTo(emp.id,r.day,r.item,{dirty:false,render:false});});restoreProtectedRecordsForCurrentMonth();ensureAllPostNightRests();if(replace)state.localDirty=false;}
  ...
  function saveStaffRecord(){const oldId=state.staffEditingId,id=oldId||nextEmployeeId(),cognome=$('#staffSurname').value.trim(),nome=$('#staffName').value.trim();if(!cognome||!nome)return toast('Dati incompleti','Cognome e nome sono obbligatori.','error');const rec=normalizeEmployee({id,cognome,nome,sesso:$('#staffSex').value,turno:$('#staffTurn').value,autista:$('#staffDriver').checked,capo:$('#staffLeader').checked,soccorritore:$('#staffRescuer').checked,l104:$('#staff104').checked,avis:$('#staffAvis').checked,congedo:$('#staffParental').checked,responsabile:$('#staffResponsible').value,sedeSolo:$('#staffSites').value==='G'?'G':'',oreSettimanali:numeric($('#staffWeeklyHours').value,0)||null,oreMensili:numeric($('#staffMonthlyHours').value,0)||null,attivo:$('#staffActive').checked});if(oldId){const idx=state.employees.findIndex(e=>e.id===oldId);state.employees[idx]=rec;}else state.employees.push(rec);state.employees.sort((a,b)=>a.turno.localeCompare(b.turno)||a.cognome.localeCompare(b.cognome));state.localDirty=true;saveState();renderAll();closeModal('staffModal');toast(oldId?'Anagrafica aggiornata':'Dipendente aggiunto',employeeName(rec));}
  ...
  function requestCalendarSave(){
    ...
    if(!url){
      openSettings();
      setTimeout(()=>$('#setAppsScript')?.focus(),120);
      return toast('Collegamento Apps Script mancante','Inserisci nelle Impostazioni l’URL Web App che termina con /exec.','error');
    }
    const count=allAssignmentRows().length;
    confirmDialog(
      'Salvare il calendario su Google Sheet?',
      `${monthLabel()} · foglio Database`,
      `Saranno scritti ${count} record. I record già presenti per ${state.month} verranno sostituiti; gli altri mesi resteranno invariati.`,
      syncCalendarGoogle
    );
  }
  ...
  async function syncCalendarGoogle(){
    const url=state.settings.appsScriptUrl.trim();
    if(!url)return requestCalendarSave();
    if(!/\/exec(?:\?|$)/.test(url)){
      return toast('URL Apps Script non valido','Usa l’indirizzo della Web App che termina con /exec, non quello dell’editor Apps Script.','error');
    }

    setCalendarSaveUi(true);
    updateSyncStatus(`Salvataggio ${monthLabel()}…`,'sync');
    ...
    try{
      const payload={
        action:'savePlan',
        month:state.month,
        rows:allAssignmentRows().map(r=>({employeeId:r.employeeId,day:r.day,assignment:r.a})),
        requirements:state.requirements,
      ...
      let data;
      try{data=JSON.parse(raw);}catch(_){throw new Error('La Web App non ha restituito JSON valido. Verifica distribuzione e autorizzazioni.');}
      if(!res.ok||!data.ok)throw new Error(data.error||`Errore HTTP ${res.status}`);

      state.localDirty=false;
      saveState();
      const savedAt=data.savedAt||new Date().toISOString();
      const localTime=new Date(savedAt).toLocaleString('it-IT',{dateStyle:'short',timeStyle:'medium'});
      updateSyncStatus(`Calendario salvato · ${localTime}`,'local');
      $('#saveResultText').innerHTML=`<strong>${esc(monthLabel())}</strong><br>${Number(data.rows||0)} record scritti nel foglio <strong>${esc(data.sheet||'Database')}</strong>.<br><span class="muted">Conferma ricevuta alle ${esc(localTime)}.</span>`;
      openModal('saveResultModal');
      toast('Calendario salvato correttamente',`${Number(data.rows||0)} record sincronizzati.`,'success');
    }catch(e){
      console.error(e);
      const message=e.name==='AbortError'
        ? 'Tempo scaduto. Verifica connessione e distribuzione Apps Script.'
        : (e.message||'Errore imprevisto');
      updateSyncStatus('Calendario non salvato','error');
      toast('Scrittura Google Sheet non riuscita',`${message} Controlla che la Web App sia distribuita come nuova versione, eseguita dal proprietario e accessibile a chiunque disponga del link.`,'error');
    }finally{
      clearTimeout(timeout);
      setCalendarSaveUi(false);
    }
  }
  ...
  function bindEvents(){
    ...
    $('#preGenMode').addEventListener('change',togglePreGenMode);
  ...
  async function sha256Hex(value){
    ...
    /* Compatibilità per apertura locale su browser senza SubtleCrypto. */
    if(value==='bosetti.danilo')return AUTH_USERS['bosetti.danilo'].passwordHash;
    if(value==='4dmin')return AUTH_USERS['admin'].passwordHash;
    return '';
  }
  ...
  function showLoginError(message='Credenziali non valide. Controlla nome utente e password.'){
    const error=$('#loginError'),shell=$('#authShell');
  ...
  function renderAuthenticatedUser(user){
    ...
    const avatar=$('#operatorAvatar'),name=$('#operatorName'),role=$('#operatorRole');
  ...
  function bindAuthentication(){
    ...
    $('#passwordToggle')?.addEventListener('click',()=>{
      const input=$('#loginPassword'),button=$('#passwordToggle');
    ...
    $('#loginPassword')?.addEventListener('keyup',event=>{
      $('#capsWarning')?.classList.toggle('visible',event.getModifierState?.('CapsLock')===true);
    });
  ...
  function bootAuthentication(){
    ...
    bindAuthentication();
    let session=null;
    try{session=JSON.parse(sessionStorage.getItem(AUTH_SESSION_KEY)||'null');}catch(_){}
    const user=session?.username?AUTH_USERS[String(session.username).toLowerCase()]:null;
    if(user){
      unlockAtlas(user);
    }else{
      sessionStorage.removeItem(AUTH_SESSION_KEY);
  ...
  async function init(){const hadLocal=loadState();restoreProtectedRecordsForCurrentMonth();$('#monthPicker').value=state.month;initRequirements();bindEvents();renderAll();if(!hadLocal){await reloadSheets({replaceDb:true,silent:true});}}
  bootAuthentication();
})();
